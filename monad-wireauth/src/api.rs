...
pub struct API<
    C: Context,
    K: AsRef<monad_secp::KeyPair> = monad_secp::KeyPair,
    M: MetricNames = DefaultMetrics,
> {
    state: State<M>,
...
impl<C: Context, K: AsRef<monad_secp::KeyPair>, M: MetricNames> API<C, K, M> {
    ...
    pub fn next_packet(&mut self) -> Option<(SocketAddr, Bytes)> {
        ...
        self.metrics[M::STATE_PACKET_QUEUE_SIZE] = self.packet_queue.len() as u64;
        result
    }
    ...
    fn insert_timer(&mut self, timer: Duration, session_id: SessionIndex) {
        ...
        self.metrics[M::STATE_TIMERS_SIZE] = self.timers.len() as u64;
    }
    ...
    fn is_under_load(
        ...
        match action {
            ...
            FilterAction::Drop => {
                self.metrics[M::RATE_LIMIT_DROP] += 1;
                false
            }
        }
    }
    ...
    pub fn decrypt<'a>(
            ...
            self.state.get_transport_mut(&receiver_index)
        {
            ...
            let (timer, plaintext) = transport
                ...
                .inspect_err(|e| {
                    track_decrypt_error_metrics::<M>(&mut self.metrics, e);
                })?;
    ...
    pub fn buffer_message(
        ...
        let new_size = initiator.buffered_bytes() + message.len();
...
fn track_decrypt_error_metrics<M: MetricNames>(metrics: &mut ExecutorMetrics, e: &SessionError) {
    metrics[M::ERROR_DECRYPT] += 1;
    match e {
        SessionError::NonceOutsideWindow { .. } => {
            metrics[M::ERROR_DECRYPT_NONCE_OUTSIDE_WINDOW] += 1;
        }
        SessionError::NonceDuplicate { .. } => {
            metrics[M::ERROR_DECRYPT_NONCE_DUPLICATE] += 1;
        }
        SessionError::InvalidMac(_) => {
            metrics[M::ERROR_DECRYPT_MAC] += 1;
        }
        _ => {
