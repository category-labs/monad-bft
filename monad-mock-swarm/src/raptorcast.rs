    ...
    PS: ProposerSchedule<PT>,
{
    ...
    /// The validator set
    pub validator_set: ValidatorSet<PT>,
...
impl<PT: PubKey, IM, OM> RaptorcastRouterConfig<PT, IM, OM> {
    pub fn new(self_id: NodeId<PT>, validator_set: ValidatorSet<PT>) -> Self {
...
where
    ...
    PS: ProposerSchedule<PT>,
{
    pub fn new_with_proposer_schedule(
        self_id: NodeId<PT>,
        validator_set: ValidatorSet<PT>,
        proposer_schedule: PS,
    ) -> Self {
        Self {
            self_id,
    ...
    PS: ProposerSchedule<PT>,
{
    ...
    fn emit_unicast(
        ...
        let deadline = time + self.config.delay;
        for chunk_id in 0..num_scaled_chunks {
            ...
            self.push_tx_event(deadline, to, WireMsg::Chunk(chunk_msg));
        }
    }
    ...
    fn emit_raptorcast(
        ...
        for chunk in chunks {
            ...
            self.push_tx_event(deadline, recipient, WireMsg::Chunk(chunk_msg));
        }
    }
    ...
    fn handle_inbound_raptorcast(
        ...
        if routing.recipient() == &self.config.self_id {
            ...
            for target in routing.rebroadcast_targets() {
                self.push_tx_event(deadline, target, WireMsg::Chunk(message.clone()));
            }
        }
