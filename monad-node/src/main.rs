...
async fn run(node_state: NodeState) -> Result<(), ()> {
    ...
    let txpool = {
        let build_standard_txpool = |score_provider, score_reader| {
        ...
        let scripted = {
            #[cfg(feature = "test-scripted-blocks")]
            {
                node_state.scripted_blocks_socket
            }
        ...
        match scripted {
            ...
            #[cfg(not(feature = "test-scripted-blocks"))]
            Some(_) => unreachable!("scripted blocks require test-scripted-blocks feature"),
            None => TxPoolExecutor::Standard(build_standard_txpool(score_provider, score_reader)),
        }
    };
