...
use monad_executor_glue::{
    LogFriendlyMonadEvent, Message, MonadEvent, PeerEntry, ResolvedPeerConfig,
};
    ...
    OM: Encodable + Clone + Send + Sync + 'static,
{
    ...
    assert_eq!(
        ...
        "self_udp_auth_port and authenticated_udp_bind_address_port must be set together"
    );
    assert_eq!(
        peer_discovery_config.self_tcp_auth_port.is_some(),
        network_config.authenticated_tcp_bind_address_port.is_some(),
        "self_tcp_auth_port and authenticated_tcp_bind_address_port must be set together"
    );
    assert!(
        !(peer_discovery_config.self_tcp_auth_port.is_some()
    ...
    let bootstrap_peers: BTreeMap<_, _> = bootstrap_nodes
        ...
        .filter_map(|peer| {
            ...
            let peer_entry = PeerEntry::from(ResolvedPeerConfig::new(peer, address));
