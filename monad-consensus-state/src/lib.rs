    ...
    CRT: ChainRevision,
{
    ...
    pub fn process_qc(
        ...
        let high_qc_block_id = self
            ...
            .qc()
            .get_block_id();
        if let Some(canonical_tip_id) = self
            .consensus
            .pending_block_tree
            .get_canonical_coherent_tip(&high_qc_block_id)
        {
            if let Some(block) = self.consensus.pending_block_tree.get_block(&canonical_tip_id) {
                cmds.push(ConsensusCommand::CommitBlocks(
    ...
    fn try_update_coherency(
        ...
        let newly_coherent_blocks = self
            .consensus
            .pending_block_tree
...
mod test {
    ...
    fn test_missing_state_root() {
        ...
        let cmds_without_proposed: Vec<_> = cmds
            .iter()
            .filter(|cmd| !matches!(cmd, ConsensusCommand::CommitBlocks(OptimisticPolicyCommit::Proposed { .. })))
            .collect();
