    ...
    CRT: ChainRevision,
{
    ...
    pub fn process_qc(
        ...
        // Emit Voted commit for the block that the QC certifies
        let qc_block_id = qc.get_block_id();
        if let Some(voted_block) = self.consensus.pending_block_tree.get_block(&qc_block_id) {
            if self.consensus.pending_block_tree.is_coherent(&qc_block_id) {
...
mod test {
    ...
    fn test_voted_commit_requires_qc() {
        ...
        let voted_rounds: Vec<_> = cmds
            ...
            .collect();
        assert!(voted_rounds.is_empty(), "No Voted commit expected before QC is observed");
