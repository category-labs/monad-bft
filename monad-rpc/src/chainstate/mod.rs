...
impl<T: Triedb> ChainState<T> {
    ...
    pub async fn get_logs(
            ...
            && has_filters
        {
            ...
            {
                Ok(logs) => {
                    match try_collect_logs_stream_with_heuristic_response_limit(
                        max_response_size,
                        from_block,
                        to_block,
                        logs,
                    ...
                    .await?
                    {
                        ...
                        Err(err) => {
                            debug!("Error getting logs from log stream with index. Falling back to unindexed method. Error: {err:?}");
                        }
                    }
                }
                Err(err) => {
                    debug!("Error creating log stream with index. Falling back to unindexed method. Error: {err:?}");
                }
            }
        }
...
async fn try_collect_logs_stream_with_heuristic_response_limit<E>(
    ...
    let num_blocks = to_block + 1 - from_block;
    ...
    while let Some(result) = stream.next().await {
        match result {
            ...
            Ok((block_number, logs)) => {
                if block_number != from_block.saturating_add(processed_blocks) {
                    error!(
                        ?from_block,
                        ?processed_blocks,
                        ?block_number,
                ...
                if heuristic_response_size >= EXTRAPOLATION_CHECK_MIN_RESPONSE_SIZE
                    || processed_blocks >= EXTRAPOLATION_CHECK_MIN_BLOCKS
                {
                    let extrapolated_heuristic_size = heuristic_response_size
                        .saturating_mul(num_blocks)
                        .saturating_div(processed_blocks);

                    let heuristic_max_response_size = (max_response_size as u64 * 2)
                        - (max_response_size as u64)
                            .saturating_mul(processed_blocks)
                            .saturating_div(num_blocks);

                    if extrapolated_heuristic_size > heuristic_max_response_size {
                        return Err(JsonRpcError::max_size_exceeded());
                    }
                }
            }
