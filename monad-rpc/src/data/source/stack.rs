...
where
    T: HistoricalDataSource + Clone,
{
    async fn try_resolve_and_then<U>(
        &self,
        ...
        for source in &self.sources {
            let pointer = match last_pointer {
                Some(pointer @ BlockPointer::Finalized(_)) => pointer,
                Some(BlockPointer::NonFinalized(_, _)) | None => {
                    match source.try_resolve(block_tag_or_hash.clone()).await {
                        Ok(Some(pointer)) => {
                            last_pointer = Some(pointer);
                            pointer
                        }
                        Ok(None) => match last_pointer {
            ...
            match f(source, pointer).await {
                ...
                Ok(None) => continue,
                Err(err) => {
                    last_err = Some(split_terminating_error(err)?);
                    continue;
                }
            }
        }
