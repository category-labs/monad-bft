...
use crate::data::source::{
    BlockCommitState, BlockPointer, DataSourceResult, HistoricalBlockData, HistoricalDataSource,
};
...
impl BlockBufferView {
    ...
    fn get_block_ref(&self, pointer: BlockPointer) -> Option<DashMapRef<'_, u64, Block>> {
        match pointer {
            BlockPointer::Finalized(block_number)
                if block_number <= self.get_latest_finalized_block_num() =>
            {
                self.block_by_height.get(&block_number)
            }
            BlockPointer::Finalized(_) => None,
...
impl HistoricalDataSource for BlockBufferView {
    async fn try_resolve_block_commit_state(
        ...
        let block_number = match commit_state {
            BlockCommitState::Finalized => {
                let block_number = self.get_latest_finalized_block_num();

                return Ok((block_number != 0).then_some(BlockPointer::Finalized(block_number)));
            }
    ...
    async fn get_block(
        ...
        Ok(Some(HistoricalBlockData {
            header: block.value().header.inner.clone(),
            header_hash_precomputed: Some(block.value().header.hash),
