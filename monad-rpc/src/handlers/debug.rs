...
mod tests {
    ...
    async fn test_global_log_index_across_nested_calls() {
        ...
        let frames = vec![
            make_frame_with_positions(1, &[0, 0]), // root: 2 logs before children
            make_frame_with_positions(2, &[0, 0, 0]), // child1: 3 logs before grandchild
        ...
        assert_eq!(first_child.logs[0].index.0, 2);
        ...
        assert_eq!(grandchild.logs[0].index.0, 5);

        let second_child = root_borrowed.calls[1].borrow();
        assert_eq!(second_child.logs.len(), 2);
        assert_eq!(second_child.logs[0].index.0, 6);
        assert_eq!(second_child.logs[1].index.0, 7);
    }
