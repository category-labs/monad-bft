...
/// finalized head with the unfinalized tip when a `chain_state` is supplied.
///
...
/// tip block's receipts from `chain_state` (exec-events buffer → triedb →
/// archive) and running the *same* [`LogFilter`] over them via
/// [`filter_unfinalized_block_logs`]. When no `chain_state` is available the
...
/// when `chain_state` is present, else the head): `latest`/`safe`/`finalized`
...
pub async fn get_logs_via_chain_data<T: Triedb>(
    service: &ChainDataService,
    chain_state: Option<&ChainState<T>>,
    filter: Filter,
    ...
    // chain_state is available, otherwise the finalized head. With neither a
    // live chain_state nor any finalized block indexed yet, there is nothing
    // to serve.
    let latest = match chain_state {
        Some(chain_state) => chain_state.get_latest_block_number(),
        None => match head {
            Some(head) => head,
    ...
    let (from_block, to_block) = match filter.block_option {
        ...
        FilterBlockOption::AtBlockHash(block_hash) => {
            // chain-data covers finalized blocks; fall back to chain_state
                ...
                .map_err(chain_data_error_to_jsonrpc)?
            {
                ...
                None => match chain_state {
                    Some(chain_state) => {
                        chain_state.resolve_block_number_by_hash(block_hash).await?
                    }
                    None => None,
                },
            };
    ...
    // Unfinalized tip: (head ..= to_block], served from chain_state.
...
/// Serves the unfinalized tip of an `eth_getLogs` range from `chain_state`,
/// appending matching logs to `logs`. Each block's receipts are fetched (and
/// bloom pre-filtered) via [`ChainState::fetch_blocks_for_logs`], its logs are
...
fn resolve_eth_block_bound(tag: Option<BlockNumberOrTag>, head: Option<u64>, latest: u64) -> u64 {
    match tag {
        ...
        // chain_state and the rest from the chain-data index.
...
// buffer→triedb→archive cascade ([`ChainState::fetch_block_for_logs`]) and
...
// last block scanned in `cursorBlock`. With no `chain_state` (e.g.
...
/// Materializes one unfinalized block from `chain_state` via the
...
async fn load_unfinalized_block<T: Triedb>(
    ...
    let Some((header, txs, receipts)) = chain_state.fetch_block_for_logs(height, |_| true).await?
    else {
        return Ok(None);
    };
...
/// chain_state), so callers can surface "no published blocks".
async fn resolve_merge_bounds<T: Triedb>(
    ...
    // chain_state is present, otherwise the finalized head.
    let head_latest = match chain_state {
        Some(chain_state) => Some(chain_state.get_latest_block_number()),
        None => head_final,
    };
...
async fn resolve_block_ref<T: Triedb>(
    ...
    if head_final.is_some_and(|head| number <= head) {
        if let Ok(Some(record)) = service.tables().blocks().load_record(number).await {
            return BlockRef {
                number,
    ...
    if let Some(chain_state) = chain_state {
        if let Ok(Some(block)) = load_unfinalized_block(chain_state, number).await {
            return BlockRef {
                number,
                hash: block.hash,
...
async fn build_merge_span<T: Triedb>(
    ...
    BlockSpan {
        from_block: resolve_block_ref(service, chain_state, bounds.head_final, bounds.from).await,
        to_block: resolve_block_ref(service, chain_state, bounds.head_final, bounds.to).await,
        cursor_block: resolve_block_ref(service, chain_state, bounds.head_final, cursor_number)
            .await,
    }
}
...
/// Scans the unfinalized tip `[lo..=hi]` (in `order`) from `chain_state`,
...
async fn scan_unfinalized_logs<T: Triedb>(
    ...
    for height in heights {
        ...
        let block = load_unfinalized_block(chain_state, height)
            .await?
...
/// from the index and the unfinalized tip (when a `chain_state` is present)
...
async fn execute_logs_query<T: Triedb>(
    ...
    let Some(bounds) = resolve_merge_bounds(service, chain_state, raw).await? else {
    ...
    let span = build_merge_span(service, chain_state, &bounds, cursor).await;
...
mod tests {
    ...
    /// unfinalized block 2 (served from a `MockTriedb` chain_state), each
    /// carrying a single tx that emits a single log. Shared by the eth and
    /// queryX tip tests.
    struct TipFixture {
        _dir: tempfile::TempDir,
        service: ChainDataService,
        chain_state: ChainState<MockTriedb>,
    ...
    async fn tip_fixture() -> TipFixture {
        ...
        // --- unfinalized block 2, served from chain_state (triedb) ---
        ...
        TipFixture {
            _dir: dir,
            service,
            chain_state: ChainState::new(None, mock, None),
            final_address,
            tip_address,
            final_tx_hash,
            tip_tx_hash,
    ...
    async fn get_logs_via_chain_data_includes_unfinalized_tip() {
        ...
        // (block 2) via chain_state, so the result spans the finalized block
        // 1 (from the index) and the unfinalized block 2 (from triedb).
        let filter = Filter::new()
            .from_block(1u64)
            .to_block(BlockNumberOrTag::Latest);
        let logs =
            get_logs_via_chain_data(&f.service, Some(&f.chain_state), filter, u32::MAX, 1000)
                .await
                .unwrap();
    ...
    async fn eth_query_logs_rejects_window_past_max_block_range() {
        ...
        // chain_state reports a tip far ahead of the (empty) index.
        let mut mock = MockTriedb::default();
        mock.set_latest_block(1000);
        let chain_state = ChainState::new(None, mock, None);
