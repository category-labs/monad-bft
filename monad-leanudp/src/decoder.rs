use std::{convert::Infallible, hash::Hash, ops::Deref, time::Instant};
    ...
    R: CryptoRng + RngCore + Clone,
{
    ...
    fn evict_one_stale_for_identity_if_limited(
        &mut self,
        identity: &I,
        now: Instant,
        selected_pool: PoolSelection,
    ) -> bool {
        let is_at_limit = match selected_pool {
            PoolSelection::Priority => self.identity_usage.is_at_priority_limit(
                identity,
                self.priority_pool.message_count(),
                self.priority_pool.max_messages(),
            ),
            PoolSelection::Regular => self.identity_usage.is_at_regular_limit(identity),
        };
        if !is_at_limit {
            return false;
        }

        let other_pool = match selected_pool {
            PoolSelection::Priority => PoolSelection::Regular,
            PoolSelection::Regular => PoolSelection::Priority,
        };

        self.evict_stale_for_identity_from_pool(selected_pool, identity, now)
            || self.evict_stale_for_identity_from_pool(other_pool, identity, now)
    }

    fn evict_stale_for_identity_from_pool(
        &mut self,
        pool: PoolSelection,
        identity: &I,
        now: Instant,
    ...
    fn decode_in_pool(
        ...
        if is_new_message {
            if self.evict_one_stale_for_identity_if_limited(&input.identity, now, selected_pool) {
                self.metrics[EvictionKind::Timeout.metric_name()] += 1;
            }
            match selected_pool {
                PoolSelection::Priority => self.identity_usage.ensure_priority_identity_capacity(
                    &input.identity,
                    self.priority_pool.message_count(),
                    self.priority_pool.max_messages(),
                )?,
                PoolSelection::Regular => self
                    .identity_usage
                    .ensure_regular_identity_capacity(&input.identity)?,
            }
        }
        ...
        let (result, pool_evicted) = match selected_pool {
            PoolSelection::Priority => {
                self.priority_pool
                    .decode_with_admission(&mut self.identity_usage, now, key, input)
            }
            PoolSelection::Regular => {
                self.regular_pool
                    .decode_with_admission(&mut self.identity_usage, now, key, input)
        ...
        T::Error: Into<DecodeError>,
    {
        ...
        if selected_pool == PoolSelection::Priority
            && matches!(result, Ok(DecodeOutcome::Complete(_)))
        {
            self.identity_usage.record_priority_service(&key.0);
        }
