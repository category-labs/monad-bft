...
use monad_chain_data::{
    ...
    Address, Bytes, FinalizedBlock, Log, LogData, MonadChainDataService, QueryLimits, Topic, B256,
};
...
/// decides whether to also scan open fragments — and these tests pin the
/// page-blob cache off (`bitmap_page_blob_entries: 0`) so every probe reaches
/// this backend. Counting
...
async fn per_page_short_circuit_skips_later_clause_fetches() {
    ...
    // Count page-blob probes on a reader that wraps the populated meta store.
    let counting = CountingMetaStore::new(store.meta.clone());
