...
const CAS: TableId = TableId::new("it_cas");
...
#[tokio::test]
#[ignore = "requires CHAIN_DATA_DYNAMO_TEST_ENDPOINT (DynamoDB Local / Alternator)"]
async fn cas_put_insert_match_conflict_matrix() {
    if endpoint().is_none() {
        return;
    }
    let store = fresh_store("cas").await;

    // Absent row, expected=None -> Applied{1}.
    assert_eq!(store.cas_get(CAS, b"head").await.unwrap(), None);
    let v1 = match store
        .cas_put(CAS, b"head", None, Bytes::from_static(b"v1"))
        .await
        .unwrap()
    {
        CasOutcome::Applied { new_version } => new_version,
        other => panic!("expected Applied, got {other:?}"),
    };
    assert_eq!(v1, CasVersion(1));
    assert_eq!(
        store.cas_get(CAS, b"head").await.unwrap(),
        Some((CasVersion(1), Bytes::from_static(b"v1")))
    );

    // Insert-if-absent on an existing row -> Conflict{current=1}.
    assert_eq!(
        store
            .cas_put(CAS, b"head", None, Bytes::from_static(b"dup"))
            .await
            .unwrap(),
        CasOutcome::Conflict {
            current_version: Some(CasVersion(1))
        }
    );

    // Stale expected -> Conflict{current=1}, row unchanged.
    assert_eq!(
        store
            .cas_put(
                CAS,
    ...
    // Matching expected -> Applied{2}.
    let v2 = match store
        .cas_put(CAS, b"head", Some(CasVersion(1)), Bytes::from_static(b"v2"))
        .await
        .unwrap()
    {
        CasOutcome::Applied { new_version } => new_version,
    ...
    assert_eq!(v2, CasVersion(2));
    assert_eq!(
        store.cas_get(CAS, b"head").await.unwrap(),
        Some((CasVersion(2), Bytes::from_static(b"v2")))
    );

    // CAS rows are physically distinct from plain kv rows even at the same key.
    assert_eq!(store.get(CAS, b"head").await.unwrap(), None);
}
