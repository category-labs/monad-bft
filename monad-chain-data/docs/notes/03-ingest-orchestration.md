...
- **`config/mod.rs`** — Operator-facing store config: meta backend (Dynamo/Alternator), blob backend (S3, Dynamo, or omitted = blob-less external-archive stores), cache budgets, reader query-runtime budgets, secret redaction (`Redacted`), and store builders shared by ingest and reader (including the Alternator batch-write-limit startup probe).
...
- **`config/reader.rs`** — `ConfiguredChainDataReader`: monomorphized meta×blob backend product behind one enum (`InMemory`/`DynamoS3`/`DynamoDynamo`/`DynamoNull` for blob-less stores) with query passthroughs; not part of ingest but built from the same config types.
