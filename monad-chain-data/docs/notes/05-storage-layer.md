...
- **put_blob** (s3.rs:365–408): single `PutObject` with a slow-send watchdog — warn at 30 s, then every 120 s; debug log if the put took ≥10 s.
- **get_blob/read_range** (s3.rs:237–283, 457–477): `read_range` pushes the range to the server (`Range: bytes=start-(end-1)`); zero-length range becomes a full point-read just to distinguish missing vs present-and-empty (s3.rs:468–471); S3 416 `InvalidRange` (start ≥ EOF) maps to `Decode("invalid blob range")`; end past EOF is clamped server-side. `NoSuchKey` ⇒ `Ok(None)`.
...
- **Cost/access patterns**:
  ...
  - Zero-length blob range reads still cost a full S3 GET.
- **Retry/timeout behavior**: dynamo batch writes retry per-chunk ≤8 times with jittered exponential backoff (50 ms→20 s) and drain `UnprocessedItems`; point reads/puts and S3 ops rely on the AWS SDK's default retry policy only. No custom operation deadlines — instead **watchdog logs** (S3 put: 30 s/120 s; dynamo batch: 10 s/60 s). An exhausted retry budget surfaces as `Backend(...)` and fails the whole `with_writes` commit.
...
8. **Zero-length range read = full GET** on S3 (can't express `bytes=x-(x-1)`; must distinguish missing vs empty).
