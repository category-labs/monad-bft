...
use crate::{
    ...
    engine::{
        ...
        bitmap::{
            ...
            BitmapPageArtifact, BitmapPageCounts, BitmapPageMeta, STREAM_PAGES_PER_SHARD,
            STREAM_PAGE_LOCAL_ID_SPAN,
        },
        digest::{self, ArtifactChecksum, EMPTY_CHECKSUM},
        family::Family,
        ingest::{
            bitmap_compaction::sealed_page_count_shards_for_batch,
            persist::PhaseAFragmentWriteFilter,
        },
        open_index::{
            insert_bitmap_page_count, insert_bitmap_set, insert_directory_set,
            OpenIndexSnapshotStatus, OpenIndexStats, OpenIndexes, OpenIndexesDelta,
            OpenIndexesEviction, OpenIndexesSnapshot,
        },
        ...
        tables::{
            DictConfig, FamilyTables, PublicationTables, StagedWrites, Tables, WriteOpCounts,
        },
    },
    ...
    primitives::{
        ...
        state::{
            BlockBlobHeader, BlockRecord, FamilyWindowRecord, LogId, PrimaryId, TraceId, TxId,
        },
    },
...
const RECOVERY_BITMAP_STREAM_CONCURRENCY: usize = 64;
const PAGE_COUNT_ACCUMULATOR_REBUILD_PAGE_CONCURRENCY: usize = 8;
const PAGE_COUNT_ACCUMULATOR_REBUILD_STREAM_CONCURRENCY: usize = 64;
const BITMAP_ARTIFACT_REPAIR_PAGE_CONCURRENCY: usize = 8;
const BITMAP_ARTIFACT_REPAIR_STREAM_CONCURRENCY: usize = 32;
...
pub struct MonadChainDataService<M: MetaStoreCas, B: BlobStore> {
    ...
    limits: QueryLimits,
    open_indexes: Arc<OpenIndexes>,
    page_count_rebuilds: PageCountRebuilds,
...
struct BitmapShardKey {
    family: Family,
    shard: u64,
}

#[derive(Default)]
struct PageCountRebuilds {
    tasks: TokioMutex<HashMap<BitmapShardKey, JoinHandle<Result<()>>>>,
}

struct PageCountRebuildSpec {
    family: Family,
    shard: u64,
    page_starts: Vec<u64>,
}

impl PageCountRebuilds {
    async fn reset(&self) {
    ...
    async fn spawn<M: MetaStoreCas>(
        &self,
        spec: PageCountRebuildSpec,
        family_tables: FamilyTables<M>,
        open_indexes: Arc<OpenIndexes>,
    ) {
        if spec.page_starts.is_empty() {
            return;
        }
        let key = BitmapShardKey {
            family: spec.family,
            shard: spec.shard,
        };
        let pages = spec.page_starts.len();
        tracing::info!(
            family = ?spec.family,
            shard = spec.shard,
            pages,
            "bitmap page count accumulator rebuild spawned"
        );
        let task = tokio::spawn(async move {
            rebuild_page_count_accumulator(spec, family_tables, open_indexes).await
        });
        let mut tasks = self.tasks.lock().await;
    ...
    async fn ensure_ready(&self, keys: Vec<BitmapShardKey>) -> Result<()> {
        for key in keys {
            ...
            tracing::info!(
                family = ?key.family,
                shard = key.shard,
                "bitmap page count accumulator rebuild wait starting"
            );
            match task.await {
                Ok(Ok(())) => {
                    tracing::info!(
                        ...
                        "bitmap page count accumulator rebuild wait completed"
                    );
                }
                Ok(Err(error)) => return Err(error),
                Err(error) => {
                    return Err(MonadChainDataError::Backend(format!(
                        "bitmap page count accumulator rebuild task failed for {:?} shard {}: {error}",
                        key.family, key.shard
                    )));
                }
            }
        }
...
async fn rebuild_page_count_accumulator<M: MetaStoreCas>(
    spec: PageCountRebuildSpec,
    family_tables: FamilyTables<M>,
    open_indexes: Arc<OpenIndexes>,
) -> Result<()> {
    ...
    let page_starts = spec.page_starts;
    let mut delta = OpenIndexesDelta::default();
    let mut page_ref_count = 0usize;
    tracing::info!(
        ?family,
        shard,
        pages = page_starts.len(),
        "bitmap page count accumulator rebuild starting"
    );
    let page_counts = stream::iter(page_starts)
        .map(|page_global_start| {
            let family_tables = family_tables.clone();
            async move {
                let page_start_local = local_page_start(page_global_start);
                let streams = family_tables
                    .load_open_bitmap_streams(page_global_start)
                    .await?;
                let stream_count = streams.len();
                let counts = stream::iter(streams)
                    .map(|stream_id| {
                        let family_tables = family_tables.clone();
                        async move {
                            let artifact = family_tables
                    ...
                    .buffer_unordered(PAGE_COUNT_ACCUMULATOR_REBUILD_STREAM_CONCURRENCY)
                    .try_collect::<Vec<_>>()
                    .await?;
                tracing::info!(
                    ?family,
                    ...
                    "bitmap page count accumulator rebuild page completed"
                );
                Ok::<_, MonadChainDataError>(counts)
            }
        })
        .buffer_unordered(PAGE_COUNT_ACCUMULATOR_REBUILD_PAGE_CONCURRENCY)
        .try_collect::<Vec<_>>()
        .await?;
    for page in page_counts {
        for (stream_id, page_start_local, count) in page {
            page_ref_count += 1;
    ...
    let pages = delta.bitmap_page_counts.len();
    open_indexes.try_apply_delta(delta)?;
    tracing::info!(
        ?family,
        shard,
        pages,
        page_refs = page_ref_count,
        elapsed_ms = rebuild_start.elapsed().as_millis() as u64,
        "bitmap page count accumulator rebuild completed"
    );
    Ok(())
}
...
pub struct BitmapArtifactRepairReport {
    pub published_head: Option<u64>,
    pub scanned_page_partitions: u64,
    pub open_stream_markers: u64,
    pub existing_artifacts: u64,
    pub repaired_artifacts: u64,
    pub rewritten_page_count_manifests: u64,
}

#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct BitmapPageCountRewriteReport {
    pub family: Option<Family>,
    pub shard: u64,
    pub scanned_page_partitions: u64,
    pub open_stream_markers: u64,
    pub loaded_artifacts: u64,
    pub rewritten_page_count_manifests: u64,
}

struct BitmapArtifactRepairWrite {
    family: Family,
    stream_id: String,
...
#[derive(Default)]
struct FamilyBitmapArtifactRepairReport {
    scanned_page_partitions: u64,
    open_stream_markers: u64,
    existing_artifacts: u64,
    repairs: Vec<BitmapArtifactRepairWrite>,
}

fn family_window_frontier(window: FamilyWindowRecord) -> Result<u64> {
    Ok(window
        .next_primary_id_exclusive()
        .map(|id| id.as_u64())
...
fn record_family_window(record: &BlockRecord, family: Family) -> FamilyWindowRecord {
    match family {
        Family::Log => record.logs,
        Family::Tx => record.txs,
        Family::Trace => record.traces,
    }
}
...
impl<M: MetaStoreCas, B: BlobStore> MonadChainDataService<M, B> {
    ...
    pub fn with_configs(
        ...
        Self {
            ...
            limits,
            open_indexes: Arc::new(OpenIndexes::default()),
            page_count_rebuilds: PageCountRebuilds::default(),
    ...
    pub fn load_open_index_snapshot_bytes(&self, bytes: &[u8]) -> Result<OpenIndexSnapshotStatus> {
        let snapshot = bincode::deserialize::<OpenIndexesSnapshot>(bytes)
            .map_err(|_| MonadChainDataError::Decode("invalid open-index snapshot"))?;
        if !self.open_indexes.replace_snapshot(snapshot) {
            return Err(MonadChainDataError::Decode(
                "unsupported open-index snapshot version",
            ));
        }
        Ok(self.open_indexes.snapshot_status())
    }

    pub fn open_index_snapshot_bytes(&self) -> Result<Vec<u8>> {
        bincode::serialize(&self.open_indexes.snapshot()).map_err(|_| {
            MonadChainDataError::Backend("failed to encode open-index snapshot".into())
        })
    }
        ...
        match &self.authority {
            ...
            ServiceAuthority::Lease(a) => {
                tracing::info!(
                    ...
                    "authority publish entering lease publish"
                );
                let result = a
                    .publish_current(
                        new_head,
                    ...
                    .await;
                match &result {
                    Ok(()) => tracing::info!(new_head, "authority publish lease publish completed"),
                    Err(error) => {
                        tracing::info!(new_head, %error, "authority publish lease publish failed")
                    }
                }
    ...
    async fn ensure_open_indexes_rebuilt(&self, current_head: Option<u64>) -> Result<()> {
        ...
        self.page_count_rebuilds.reset().await;
        ...
        let (log_rebuild, tx_rebuild, trace_rebuild) = futures::try_join!(
            self.rebuild_family_open_indexes(Family::Log, record.logs, head),
            self.rebuild_family_open_indexes(Family::Tx, record.txs, head),
            self.rebuild_family_open_indexes(Family::Trace, record.traces, head),
        )?;
        let mut delta = OpenIndexesDelta::default();
        delta.merge(log_rebuild.0);
        delta.merge(tx_rebuild.0);
        delta.merge(trace_rebuild.0);
        self.open_indexes.replace_rebuilt(Some(head), delta);
        for spec in [log_rebuild.1, tx_rebuild.1, trace_rebuild.1]
            .into_iter()
            .flatten()
        {
            let family_tables = self.tables.family(spec.family).clone();
            self.page_count_rebuilds
                .spawn(spec, family_tables, self.open_indexes.clone())
                .await;
        }
        Ok(())
    }
    ...
    async fn rebuild_family_open_indexes(
        ...
        window: crate::primitives::state::FamilyWindowRecord,
        published_head: u64,
    ) -> Result<(OpenIndexesDelta, Option<PageCountRebuildSpec>)> {
        let mut delta = OpenIndexesDelta::default();
        ...
        let directory_fragments = family_tables
            ...
            .await?;
        insert_directory_set(&mut delta, family, open_bucket, directory_fragments);
        ...
        let fragment_sets = stream::iter(streams)
            .map(|stream_id| async move {
                let fragments = family_tables
                    ...
                    .await?;
                Ok::<_, MonadChainDataError>((stream_id, fragments))
            })
            .buffer_unordered(RECOVERY_BITMAP_STREAM_CONCURRENCY)
            .try_collect::<Vec<_>>()
            .await?;
        for (stream_id, fragments) in fragment_sets {
            ...
            insert_bitmap_set(&mut delta, family, stream_id, page_start_local, fragments);
        }

        let current_shard = next_primary_id / SHARD_LOCAL_ID_SPAN;
        let current_shard_start = current_shard * SHARD_LOCAL_ID_SPAN;
        let page_starts = (current_shard_start..open_page)
            .step_by(STREAM_PAGE_LOCAL_ID_SPAN as usize)
            .collect::<Vec<_>>();
        let page_count_rebuild = (!page_starts.is_empty()).then_some(PageCountRebuildSpec {
            family,
            shard: current_shard,
            page_starts,
        });
        Ok((delta, page_count_rebuild))
    }

    pub async fn repair_missing_bitmap_artifacts(&self) -> Result<BitmapArtifactRepairReport> {
        let published_head = self.publication.load_published_head().await?;
        let Some(head) = published_head else {
            return Ok(BitmapArtifactRepairReport {
                published_head,
                ..Default::default()
            });
        };
        ...
        let mut report = BitmapArtifactRepairReport {
            published_head,
            ..Default::default()
        };
        ...
        for (family, window) in [
            ...
            tracing::info!(
                ?family,
                published_head = head,
                frontier = family_window_frontier(window)?,
                "bitmap artifact repair family scan starting"
            );
            let family_report = self
                .repair_missing_family_bitmap_artifacts(family, window, head)
                .await?;
            tracing::info!(
                ?family,
                published_head = head,
                scanned_page_partitions = family_report.scanned_page_partitions,
                open_stream_markers = family_report.open_stream_markers,
                existing_artifacts = family_report.existing_artifacts,
                repaired_artifacts = family_report.repairs.len(),
                elapsed_ms = family_start.elapsed().as_millis() as u64,
                "bitmap artifact repair family scan completed"
            );
            report.scanned_page_partitions += family_report.scanned_page_partitions;
            report.open_stream_markers += family_report.open_stream_markers;
            report.existing_artifacts += family_report.existing_artifacts;
            repairs.extend(family_report.repairs);
        }

        let repaired_meta: BTreeMap<(Family, String, u32), BitmapPageMeta> = repairs
            .iter()
            .map(|repair| {
                (
                    (
                        repair.family,
                        repair.stream_id.clone(),
                        repair.page_start_local,
                    ),
                    repair.artifact.meta,
                )
            })
            .collect();
        let mut affected_sealed_shards = BTreeSet::<(Family, u64, String)>::new();
        for repair in &repairs {
            let page_global_start =
                stream_page_global_start(&repair.stream_id, repair.page_start_local)?;
            let shard = page_global_start / SHARD_LOCAL_ID_SPAN;
            let family_frontier =
                family_window_frontier(record_family_window(&record, repair.family))?;
            if (shard + 1) * SHARD_LOCAL_ID_SPAN <= family_frontier {
                affected_sealed_shards.insert((repair.family, shard, repair.stream_id.clone()));
            }
        }

        let mut manifests = Vec::new();
        for (family, _shard, stream_id) in affected_sealed_shards {
            let family_tables = self.tables.family(family);
            let mut pairs = Vec::new();
            for page_idx in 0..STREAM_PAGES_PER_SHARD {
                let page_start_local = page_idx * STREAM_PAGE_LOCAL_ID_SPAN;
                let count = if let Some(meta) =
                    repaired_meta.get(&(family, stream_id.clone(), page_start_local))
                {
                    meta.count
                } else {
                    family_tables
                        .load_bitmap_page_artifact(&stream_id, page_start_local)
                        .await?
                        .map(|artifact| artifact.meta.count)
                        .unwrap_or(0)
                };
            ...
            if !counts.pages.is_empty() {
                manifests.push((family, stream_id, counts));
            }
        }

        report.repaired_artifacts = repairs.len() as u64;
        report.rewritten_page_count_manifests = manifests.len() as u64;
        if !repairs.is_empty() || !manifests.is_empty() {
            self.tables
                .with_writes(|w| {
                    let repairs = &repairs;
                    ...
                    Box::pin(async move {
                        ...
                        for (family, stream_id, counts) in manifests {
                            self.tables
                                .family(*family)
                                .bitmap()
    ...
    pub async fn rewrite_bitmap_page_count_manifests(
        &self,
        family: Family,
        shard: u64,
    ) -> Result<BitmapPageCountRewriteReport> {
        let family_tables = self.tables.family(family);
        let shard_start = shard * SHARD_LOCAL_ID_SPAN;
        let page_starts = (shard_start..shard_start + SHARD_LOCAL_ID_SPAN)
            .step_by(STREAM_PAGE_LOCAL_ID_SPAN as usize)
            .collect::<Vec<_>>();
        ...
        tracing::info!(
            ?family,
            shard,
            page_partitions = page_starts.len(),
            "bitmap page count manifest rewrite starting"
        );

        let page_counts = stream::iter(page_starts)
            .map(|page_global_start| async move {
                let page_start_local = local_page_start(page_global_start);
                let streams = family_tables
                    .load_open_bitmap_streams(page_global_start)
                    .await?;
                ...
                let counts = stream::iter(streams)
                    .map(|stream_id| async move {
                        let artifact = family_tables
                            .load_bitmap_page_artifact(&stream_id, page_start_local)
                            .await?
                            .ok_or_else(|| MonadChainDataError::SealedShardPageMissingArtifact {
                                stream_id: stream_id.clone(),
                                page_start_local,
                            })?;
                        Ok::<_, MonadChainDataError>((
                ...
                tracing::info!(
                    ...
                    page_start_local,
                    streams = stream_count,
                    "bitmap page count manifest rewrite page loaded"
                );
                Ok::<_, MonadChainDataError>((stream_count, counts))
            })
            .buffer_unordered(PAGE_COUNT_ACCUMULATOR_REBUILD_PAGE_CONCURRENCY)
            .try_collect::<Vec<_>>()
            .await?;

        let mut pages_by_stream = BTreeMap::<String, Vec<(u32, u32)>>::new();
        let mut open_stream_markers = 0u64;
        let mut loaded_artifacts = 0u64;
        for (stream_count, page) in page_counts {
            open_stream_markers += stream_count as u64;
            for (stream_id, page_start_local, count) in page {
                loaded_artifacts += 1;
                pages_by_stream
                    .entry(stream_id)
                    .or_default()
                    .push((page_start_local, count));
            }
        }
        let manifests = pages_by_stream
            .into_iter()
            .filter_map(|(stream_id, pairs)| {
                let counts = BitmapPageCounts::from_pairs(pairs);
                (!counts.pages.is_empty()).then_some((stream_id, counts))
            })
            .collect::<Vec<_>>();
        ...
        self.tables
            .with_writes(|w| {
                let manifests = &manifests;
                Box::pin(async move {
                    for (stream_id, counts) in manifests {
                        family_tables
                            .bitmap()
                            .stage_page_counts(w, stream_id, counts);
                    }
        ...
        tracing::info!(
            ...
            open_stream_markers,
            loaded_artifacts,
            rewritten_page_count_manifests = manifests.len(),
            elapsed_ms = rewrite_start.elapsed().as_millis() as u64,
            "bitmap page count manifest rewrite completed"
        );
        Ok(BitmapPageCountRewriteReport {
            family: Some(family),
            shard,
            scanned_page_partitions: STREAM_PAGES_PER_SHARD as u64,
            open_stream_markers,
            loaded_artifacts,
            rewritten_page_count_manifests: manifests.len() as u64,
        })
    }

    async fn repair_missing_family_bitmap_artifacts(
        &self,
        family: Family,
        window: FamilyWindowRecord,
        published_head: u64,
    ) -> Result<FamilyBitmapArtifactRepairReport> {
        let frontier = family_window_frontier(window)?;
        let open_page = global_page_start(frontier);
        let current_shard_start = (frontier / SHARD_LOCAL_ID_SPAN) * SHARD_LOCAL_ID_SPAN;
        if open_page <= current_shard_start {
            return Ok(FamilyBitmapArtifactRepairReport::default());
        }
        let page_starts = (current_shard_start..open_page)
            .step_by(STREAM_PAGE_LOCAL_ID_SPAN as usize)
            .collect::<Vec<_>>();
        tracing::info!(
            ...
            current_shard_start,
            open_page,
            scanned_page_partitions = page_starts.len(),
            skipped_historical_page_partitions =
                current_shard_start / u64::from(STREAM_PAGE_LOCAL_ID_SPAN),
            "bitmap artifact repair family current-shard range selected"
        );
        stream::iter(page_starts)
            .map(|page_global_start| async move {
                self.repair_missing_family_bitmap_page_artifacts(
                    family,
                    page_global_start,
                    published_head,
                )
            ...
            .buffer_unordered(BITMAP_ARTIFACT_REPAIR_PAGE_CONCURRENCY)
            .try_fold(
                FamilyBitmapArtifactRepairReport::default(),
                |mut acc, page| async move {
                    acc.scanned_page_partitions += page.scanned_page_partitions;
                    acc.open_stream_markers += page.open_stream_markers;
                    acc.existing_artifacts += page.existing_artifacts;
                    acc.repairs.extend(page.repairs);
                    Ok(acc)
                },
            )
            .await
    ...
    async fn repair_missing_family_bitmap_page_artifacts(
        &self,
        family: Family,
        ...
        let streams = family_tables
            .load_open_bitmap_streams(page_global_start)
            .await?;
        let mut report = FamilyBitmapArtifactRepairReport {
            scanned_page_partitions: 1,
            open_stream_markers: streams.len() as u64,
            ..Default::default()
        };
        let stream_count = streams.len();
        if stream_count == 0 {
            tracing::debug!(
                ...
                page_global_start,
                page_start_local,
                "bitmap artifact repair page partition empty"
            );
            return Ok(report);
        }
        let page_start = Instant::now();
        let page_repairs = stream::iter(streams)
            .map(|stream_id| async move {
                if family_tables
                    .load_bitmap_page_artifact(&stream_id, page_start_local)
                    .await?
                    .is_some()
                {
                    return Ok::<_, MonadChainDataError>((true, None));
                }
                let fragments = family_tables
                    .list_bitmap_fragments_for_rebuild(&stream_id, page_start_local, published_head)
                    .await?;
                ...
                let (meta, bitmap_blob) =
                    compact_bitmap_page(page_start_local, fragments.values())?;
                Ok((
                    false,
                    Some(BitmapArtifactRepairWrite {
                        family,
                        stream_id,
                        page_start_local,
                        artifact: BitmapPageArtifact { meta, bitmap_blob },
                    }),
                ))
            })
            .buffer_unordered(BITMAP_ARTIFACT_REPAIR_STREAM_CONCURRENCY)
            .try_collect::<Vec<_>>()
            .await?;
        for (existed, repair) in page_repairs {
            if existed {
                report.existing_artifacts += 1;
            }
        ...
        tracing::info!(
            ...
            page_global_start,
            page_start_local,
            open_stream_markers = stream_count,
            existing_artifacts = report.existing_artifacts,
            repaired_artifacts = report.repairs.len(),
            elapsed_ms = page_start.elapsed().as_millis() as u64,
            "bitmap artifact repair page partition completed"
        );
        Ok(report)
    }
    ...
    /// Initializes the write-side planning frontier before the first batch is
    /// planned. Calling this during process startup makes any required recovery
    /// explicit instead of hiding it inside the first `plan_ingest_blocks` call.
    pub async fn prepare_ingest_planning_state(&self) -> Result<()> {
        let mut planning_state = self.planning_state.lock().await;
        if planning_state.is_some() {
            return Ok(());
        }

        // Cold cache: take (or renew) write authority to learn the
        // authoritative head and whether ownership transitioned. The lease is
        // cached in the authority; publish-time revalidation (D1 option A)
        // re-checks it inside the CAS rather than holding a guard across
        ...
        let previous = match current_head {
            Some(head) => Some(self.tables.blocks().load_record(head).await?.ok_or(
                MonadChainDataError::MissingData("missing published head block record"),
            )?),
            None => None,
        };
    ...
    /// Benchmark-only escape hatch for suffix ingest on an empty store. This
    /// seeds the planner as if `start_block - 1` had already been published,
    /// using `start_parent_hash` as that synthetic predecessor's hash and
    /// zero-valued family primary-id frontiers. The resulting store is only
    /// internally consistent for the ingested suffix; it is not a full-chain
    /// index.
    pub async fn seed_synthetic_ingest_start(
        &self,
        start_block: u64,
        start_parent_hash: Hash32,
    ) -> Result<()> {
        if start_block <= 1 {
            return Err(MonadChainDataError::InvalidRequest(
                "synthetic ingest start requires start block > 1",
            ));
        }
        if self.publication.load_published_head().await?.is_some() {
            return Err(MonadChainDataError::InvalidRequest(
                "synthetic ingest start requires an empty publication head",
            ));
        }
        ...
        if planning_state.is_some() {
            return Err(MonadChainDataError::InvalidRequest(
                "synthetic ingest start must be seeded before planning begins",
            ));
        }

        let synthetic_head = start_block - 1;
        *planning_state = Some(IngestPlanningState {
            head: Some(synthetic_head),
            previous: Some(BlockRecord {
                block_number: synthetic_head,
                block_hash: start_parent_hash,
                parent_hash: Hash32::default(),
                logs: crate::primitives::state::FamilyWindowRecord {
                    first_primary_id: LogId::ZERO.into(),
                    count: 0,
                },
                txs: crate::primitives::state::FamilyWindowRecord {
                    first_primary_id: TxId::ZERO.into(),
                    count: 0,
                },
                traces: crate::primitives::state::FamilyWindowRecord {
                    first_primary_id: TraceId::ZERO.into(),
                    count: 0,
                },
    ...
    pub async fn plan_ingest_blocks(
        ...
        let plan_total_start = Instant::now();
        let batch_blocks = blocks.len();
        let first_block_number = blocks[0].block_number();
        let last_block_number = blocks[batch_blocks - 1].block_number();
        tracing::info!(
            blocks = batch_blocks,
            first_block = first_block_number,
            last_block = last_block_number,
            "ingest plan starting"
        );
        ...
        self.prepare_ingest_planning_state().await?;
        let mut planning_state = self.planning_state.lock().await;
        ...
        tracing::info!(
            ...
            first_block = first_block_number,
            last_block = last_block_number,
            "ingest plan stage A starts/window build starting"
        );
        ...
        for epoch in first_epoch..=last_epoch {
            ...
            tracing::info!(
                ...
                "ingest plan epoch codecs ready"
            );
        }
        let epoch_codecs_ref = &epoch_codecs;

        let parallel_plan_start = Instant::now();
        tracing::info!(
            blocks = batch_blocks,
            first_block = first_block_number,
            last_block = last_block_number,
            "ingest plan stage A parallel build starting"
        );
        let planned: Vec<StageAPlanOutput> = blocks
            ...
            .map(|(block, starts)| {
                ...
                let (log_plan, (tx_plan, trace_plan)) = rayon::join(
                    || LogIngestPlan::build(block, starts.log, log_codec_ref),
                    || {
                        rayon::join(
                            || TxIngestPlan::build(block, starts.tx, tx_codec_ref),
                            || TraceIngestPlan::build(block, starts.trace, trace_codec_ref),
                        )
                    },
                );
                let mut log_plan = log_plan?;
                let mut tx_plan = tx_plan?;
                let mut trace_plan = trace_plan?;
        ...
        tracing::info!(
            ...
            first_block = first_block_number,
            last_block = last_block_number,
            elapsed_ms = parallel_plan_start.elapsed().as_millis() as u64,
            "ingest plan stage A parallel build completed"
        );
        ...
        tracing::info!(
            ...
            first_block = first_block_number,
            last_block = last_block_number,
            stage_a_ms = timings.stage_a_ms,
            "ingest plan stage A completed"
        );
        ...
        tracing::info!(
            ...
            first_block = first_block_number,
            last_block = last_block_number,
            bitmap_fragments = phase_a_delta.bitmap_fragments.len(),
            bitmap_open_streams = phase_a_delta.bitmap_open_streams.len(),
            directory_fragments = phase_a_delta.directory_fragments.len(),
            elapsed_ms = phase_a_delta_start.elapsed().as_millis() as u64,
            "ingest plan phase A delta built"
        );
        ...
        let mut page_count_rebuild_dependencies = Vec::new();
        page_count_rebuild_dependencies.extend(
            sealed_page_count_shards_for_batch(&log_touched_bitmap_per_block, &log_ranges)
                .into_iter()
                .map(|shard| BitmapShardKey {
                    family: Family::Log,
                    shard,
                }),
        );
        page_count_rebuild_dependencies.extend(
            sealed_page_count_shards_for_batch(&tx_touched_bitmap_per_block, &tx_ranges)
                .into_iter()
                .map(|shard| BitmapShardKey {
                    family: Family::Tx,
                    shard,
        ...
        page_count_rebuild_dependencies.extend(
            sealed_page_count_shards_for_batch(&trace_touched_bitmap_per_block, &trace_ranges)
                .into_iter()
                .map(|shard| BitmapShardKey {
                    family: Family::Trace,
                    shard,
        ...
        self.page_count_rebuilds
            .ensure_ready(page_count_rebuild_dependencies)
            .await?;
        ...
        let phase_a_stage = self.tables.stage_writes(|w| {
            Box::pin(async move {
                ...
                tracing::info!(
                    blocks = blocks_ref.len(),
                    elapsed_ms = stage_start.elapsed().as_millis() as u64,
                    "ingest plan phase A write staging closure completed"
                );
                Ok(())
            })
        });
        ...
        let stage_b_plan = async {
            ...
            tracing::info!(
                ...
                first_block = first_block_number,
                last_block = last_block_number,
                "ingest plan phase B reads starting"
            );
            let log_dir_future = async {
                let start = Instant::now();
                tracing::info!(family = ?Family::Log, kind = "directory", "ingest plan phase B subplan starting");
                let plan = logs
                    .plan_directory_compactions(&projected_open_indexes, &log_ranges)
                    .await?;
                tracing::info!(
                    family = ?Family::Log,
                    kind = "directory",
                    ...
                    "ingest plan phase B subplan completed"
                );
                Ok::<_, MonadChainDataError>(plan)
            };
            let log_bitmap_future = async {
                let start = Instant::now();
                tracing::info!(family = ?Family::Log, kind = "bitmap", "ingest plan phase B subplan starting");
                let plan = logs
                ...
                tracing::info!(
                    ...
                    compacted_pages = plan.compacted_pages.len(),
                    open_stream_writes = plan.open_stream_writes.len(),
                    page_count_manifests = plan.page_count_manifests.len(),
                    elapsed_ms = start.elapsed().as_millis() as u64,
                    "ingest plan phase B subplan completed"
                );
                Ok::<_, MonadChainDataError>(plan)
            };
            let tx_dir_future = async {
                let start = Instant::now();
                tracing::info!(family = ?Family::Tx, kind = "directory", "ingest plan phase B subplan starting");
                let plan = txs
                    .plan_directory_compactions(&projected_open_indexes, &tx_ranges)
                    .await?;
                tracing::info!(
                    family = ?Family::Tx,
                    kind = "directory",
            ...
            let tx_bitmap_future = async {
                let start = Instant::now();
                tracing::info!(family = ?Family::Tx, kind = "bitmap", "ingest plan phase B subplan starting");
                let plan = txs
                    .plan_bitmap_compactions(
                        &projected_open_indexes,
                        committed_open_indexes,
            ...
            let trace_dir_future = async {
                let start = Instant::now();
                tracing::info!(family = ?Family::Trace, kind = "directory", "ingest plan phase B subplan starting");
                let plan = traces
                    .plan_directory_compactions(&projected_open_indexes, &trace_ranges)
                    .await?;
                tracing::info!(
                    family = ?Family::Trace,
                    kind = "directory",
            ...
            let trace_bitmap_future = async {
                let start = Instant::now();
                tracing::info!(family = ?Family::Trace, kind = "bitmap", "ingest plan phase B subplan starting");
                let plan = traces
                    .plan_bitmap_compactions(
                        &projected_open_indexes,
                        committed_open_indexes,
            ...
            tracing::info!(
                ...
                first_block = first_block_number,
                last_block = last_block_number,
                elapsed_ms = reads_start.elapsed().as_millis() as u64,
                "ingest plan phase B reads completed"
            );
        ...
        tracing::info!(
            ...
            first_block = first_block_number,
            last_block = last_block_number,
            phase_a_write_ops = timings.phase_a_write_counts.total_ops(),
            phase_a_write_bytes = timings.phase_a_write_counts.total_bytes(),
            "ingest plan phase A staging completed"
        );
        ...
        let mut phase_b_delta = OpenIndexesDelta::default();
        if phase_b_has_writes {
            ...
            tracing::info!(
                ...
                first_block = first_block_number,
                last_block = last_block_number,
                stage_b_ms = timings.stage_b_ms,
                phase_b_write_ops = timings.phase_b_write_counts.total_ops(),
                phase_b_write_bytes = timings.phase_b_write_counts.total_bytes(),
                "ingest plan phase B staging completed"
            );
            ...
            phase_b_delta.merge(log_bitmap_plan.page_count_delta.clone());
            phase_b_delta.merge(tx_bitmap_plan.page_count_delta.clone());
            phase_b_delta.merge(trace_bitmap_plan.page_count_delta.clone());
        ...
        self.open_indexes.try_apply_delta(phase_a_delta)?;
        if phase_b_has_writes {
            self.open_indexes.try_apply_delta(phase_b_delta)?;
        ...
        *planning_state = Some(IngestPlanningState {
        ...
        tracing::info!(
            ...
            first_block = first_block_number,
            last_block = last_block_number,
            total_ms = plan_total_start.elapsed().as_millis() as u64,
            stage_a_ms = timings.stage_a_ms,
            reads_ms = timings.reads_ms,
            stage_b_ms = timings.stage_b_ms,
            phase_b_skipped = timings.phase_b_skipped,
            "ingest plan completed"
        );
        Ok(Some(IngestPlan {
            outcomes,
            timings,
    ...
    pub async fn publish_ingest_advance(&self, publication: PublicationAdvance) -> Result<u64> {
        ...
        tracing::info!(
            ...
            "publish ingest advance starting"
        );
        ...
        tracing::info!(
            new_head = publication.new_head,
            cas_ms,
            "publish ingest advance completed"
        );
        Ok(cas_ms)
    }
