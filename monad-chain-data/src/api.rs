...
use crate::{
    ...
    query::{indexed::execute_indexed_log_query, runner::execute_unfiltered_block_query},
...
impl<M: MetaStore, B: BlobStore> MonadChainDataService<M, B> {
    ...
    pub async fn ingest_block(&self, block: FinalizedBlock) -> Result<IngestOutcome> {
        ...
        blocks.validate_continuity(&block, current_head).await?;
        let next_log_id = match current_head {
            Some(head) => blocks
                .load_record(head)
                .await?
    ...
    pub async fn query_logs(&self, request: QueryLogsRequest) -> Result<QueryLogsResponse> {
        ...
        execute_unfiltered_block_query(&self.tables, &request, window).await
    }
}
