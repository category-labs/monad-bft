...
impl<M: MetaStoreCas> LeaseAuthority<M> {
    ...
    async fn renew_if_needed(
        ...
        tracing::info!(
            ...
            renew_threshold_blocks = self.renew_threshold_blocks,
            lease_version = lease.version.0,
            "publication renew check"
        );
        if !lease.needs_renewal(
            ...
            tracing::info!(
                ...
                lease_valid_through_block = lease.lease_valid_through_block,
                lease_version = lease.version.0,
                "publication renew skipped"
            );
            return Ok(lease);
        }

        tracing::info!("publication renew loading current lease state");
        ...
        loop {
            ...
            tracing::info!(
                ...
                new_lease_valid_through_block = extended,
                "publication renew CAS starting"
            );
            match self.cas_state(Some(current.version), next).await? {
                CasOutcome::Applied { new_version } => {
                    tracing::info!(
                        ...
                        new_lease_valid_through_block = extended,
                        "publication renew CAS applied"
                    );
                    return Ok(self.lease_from(next, new_version));
                }
                CasOutcome::Conflict { .. } => {
                    tracing::info!("publication renew CAS conflict; reloading lease state");
                    current = self.load_lease_or_conflict().await?;
                }
            }
        }
    ...
    pub async fn publish_current(
        ...
        tracing::info!(
            new_head,
            observed_upstream_finalized_block,
            "publication publish waiting for lease lock"
        );
        let mut guard = self.lease.lock().await;
        tracing::info!(new_head, "publication publish acquired lease lock");
        let mut lease = (*guard).ok_or(MonadChainDataError::LeaseLost)?;
        tracing::info!(
            new_head,
            current_head = lease.indexed_finalized_head,
            lease_valid_through_block = lease.lease_valid_through_block,
            lease_version = lease.version.0,
            "publication publish loaded cached lease"
        );
        let observed_upstream_finalized_block =
            require_observed_finalized_block(observed_upstream_finalized_block)?;
        if observed_upstream_finalized_block > lease.lease_valid_through_block {
            tracing::info!(
                new_head,
                observed_upstream_finalized_block,
                lease_valid_through_block = lease.lease_valid_through_block,
                "publication publish cached lease expired; reloading before publish"
            );
            let current = self.load_lease_or_conflict().await?;
            if current.owner_id != lease.owner_id || current.session_id != lease.session_id {
                return Err(MonadChainDataError::LeaseLost);
            }
            lease = current;
        ...
        tracing::info!(
            ...
            lease_valid_through_block = lease.lease_valid_through_block,
            lease_version = lease.version.0,
            "publication publish lease ready"
        );
        ...
        let extended = self.lease_valid_through_block(observed_upstream_finalized_block);
        if extended > next_state.lease_valid_through_block {
            next_state.lease_valid_through_block = extended;
        }
        tracing::info!(
            new_head,
            expected_version = lease.version.0,
            new_lease_valid_through_block = next_state.lease_valid_through_block,
            "publication publish CAS starting"
        );
            ...
            CasOutcome::Applied { new_version } => {
                *guard = Some(self.lease_from(next_state, new_version));
                tracing::info!(
                    new_head,
                    new_version = new_version.0,
                    "publication publish CAS applied"
                );
                Ok(())
            }
            CasOutcome::Conflict { .. } => {
                tracing::info!(
                    new_head,
                    expected_version = lease.version.0,
                    "publication publish CAS conflict; loading current state"
                );
...
mod tests {
    ...
    #[test]
    fn publish_extends_same_session_even_after_cached_lease_bound_passed() {
        block_on(async {
            let store = InMemoryMetaStore::default();
            let authority = authority(&store, 7, [9u8; 16], 50, 10);
            let sess = authority.begin_write(Some(100)).await.expect("acquire");
            sess.publish(3, ArtifactChecksum::repeat_byte(0x33), Some(100))
                .await
            ...
            let sess = authority
                ...
                .await
                .expect("same session");
            let checksum = ArtifactChecksum::repeat_byte(0x44);
            sess.publish(4, checksum, Some(150))
                .await
                .expect("same session can publish and extend after cached lease bound is passed");

            let after = load_state(&store).await;
            assert_eq!(after.owner_id, 7);
            assert_eq!(after.session_id, [9u8; 16]);
            assert_eq!(after.indexed_finalized_head, 4);
            assert_eq!(after.lease_valid_through_block, 199);
            assert_eq!(after.head_artifact_checksum, checksum);
        });
    }
