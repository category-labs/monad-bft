...
use crate::{
    ...
    kernel::{
        bitmap::{
            ...
            BitmapFragmentWrite, STREAM_PAGE_LOCAL_ID_SPAN,
        },
        tables::LogTables,
    },
...
async fn compact_page_streams<M: MetaStore, B: BlobStore>(
    ...
    for stream_id in streams {
        let fragments = logs
            ...
            .await?;
        // A stream only enters the open-page inventory after fragment writes for
        // that page, so an empty merge here indicates corrupted compaction state.
        let Some((meta, bitmap_blob)) = compact_bitmap_page(page_start_local, &fragments)? else {
            return Err(MonadChainDataError::MissingData(
                "missing sealed log bitmap page fragments",
            ));
        };
...
fn build_compaction_plan(
    ...
    let mut current_page = start_open_page;
    let mut sealed_pages = Vec::new();

    loop {
        let mut open_streams = if current_page == start_open_page {
            previous_open_streams.clone()
        } else {
            BTreeSet::new()
        };
        if let Some(touched) = touched_by_page.get(&current_page) {
            open_streams.extend(touched.iter().cloned());
        }

        if current_page == final_open_page_start {
            return BitmapCompactionPlan {
                sealed_pages,
                final_open_page_start,
                final_open_streams: open_streams,
            };
        }

        if !open_streams.is_empty() {
            sealed_pages.push((current_page, open_streams));
        }
        current_page = current_page.saturating_add(u64::from(STREAM_PAGE_LOCAL_ID_SPAN));
    }
}
...
mod tests {
    ...
    #[test]
    fn compaction_plan_walks_across_three_shards() {
