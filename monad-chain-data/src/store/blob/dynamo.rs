...
/// monad-archive's Dynamo item schema (its `kvstore::dynamodb`): partition
/// key attribute `tx_hash` (string), value attribute `data` (binary), and for
/// chunked values a `chunks` count on the main item with the bytes in
/// `{key}_chunk_{i}` items of [`ARCHIVE_DYNAMO_CHUNK_SIZE`] bytes each.
const ARCHIVE_PARTITION_KEY: &str = "tx_hash";
const ARCHIVE_DATA_ATTR: &str = "data";
const ARCHIVE_CHUNKS_ATTR: &str = "chunks";

/// monad-archive's Dynamo chunk size (its `kvstore::dynamodb::CHUNK_SIZE`).
/// A wire contract: it sets the byte->chunk-index mapping of existing data.
pub const ARCHIVE_DYNAMO_CHUNK_SIZE: usize = 350 * 1024;

/// Construction parameters for [`DynamoExternalBlobReader`].
#[derive(Debug, Clone)]
pub struct DynamoExternalBlobReaderConfig {
    ...
    pub profile: Option<String>,
    pub credentials: Option<DynamoCredentials>,
    /// Cap on concurrent item reads.
    pub max_concurrency: usize,
}

/// Read-only, raw-key access to a monad-archive block-data table on
/// DynamoDB/Alternator — the dynamo counterpart of
/// [`crate::store::S3ExternalBlobReader`] /
/// [`crate::store::MongoExternalBlobReader`]. Keys are the archive's own
/// object keys (`block/000000000123`); the chain-data pk/sk layout above does
/// NOT apply. Range reads fetch only the covering chunk items (the cross-crate
/// round-trip test in monad-archive guards the schema mirror).
pub struct DynamoExternalBlobReader {
    client: Client,
    table: String,
    read_limit: Arc<tokio::sync::Semaphore>,
}

/// One archive item, before chunk resolution.
enum ArchiveItem {
    Value(Bytes),
    Chunked(usize),
}

impl DynamoExternalBlobReader {
    pub async fn new(config: DynamoExternalBlobReaderConfig) -> Result<Self> {
        let DynamoExternalBlobReaderConfig {
            table,
            endpoint_url,
            region,
            profile,
        ...
        Ok(Self {
            client,
            table,
            read_limit: Arc::new(tokio::sync::Semaphore::new(max_concurrency.max(1))),
        })
    }

    async fn get_item(&self, key: &str) -> Result<Option<ArchiveItem>> {
        let _permit = self
            .read_limit
            .acquire()
            .await
            .expect("read-limit semaphore is never closed");
        let resp = self
            .client
            .get_item()
            .table_name(&self.table)
            .key(ARCHIVE_PARTITION_KEY, AttributeValue::S(key.to_owned()))
            .send()
            .await
            .map_err(|e| backend_err("archive get_item", e))?;
        let Some(mut item) = resp.item else {
            return Ok(None);
        };
        if let Some(AttributeValue::B(data)) = item.remove(ARCHIVE_DATA_ATTR) {
            return Ok(Some(ArchiveItem::Value(Bytes::from(data.into_inner()))));
        }
        if let Some(AttributeValue::N(count)) = item.remove(ARCHIVE_CHUNKS_ATTR) {
            let count: usize = count.parse().map_err(|_| {
                MonadChainDataError::Backend(format!(
                    "dynamo archive item {key} has a non-numeric chunk count"
                ))
            })?;
            if count == 0 {
                return Err(MonadChainDataError::Backend(format!(
                    "dynamo archive item {key} has a zero chunk count"
                )));
            }
            return Ok(Some(ArchiveItem::Chunked(count)));
        }
        Err(MonadChainDataError::Backend(format!(
            "dynamo archive item {key} has neither data nor chunks"
        )))
    }
    ...
    async fn read_range_at(
        ...
        use crate::store::blob::external_chunks::{
            archive_chunk_key, assemble_chunked_range, covering_chunks, slice_range,
        };

        match self.get_item(key).await? {
            None => Ok(None),
            Some(ArchiveItem::Value(bytes)) => Ok(Some(slice_range(&bytes, start, end_exclusive)?)),
            Some(ArchiveItem::Chunked(chunk_count)) => {
                let Some(fetch) =
                    covering_chunks(start, end_exclusive, chunk_count, ARCHIVE_DYNAMO_CHUNK_SIZE)?
                else {
                    return Ok(Some(Bytes::new()));
                };
                let chunks = futures::future::try_join_all(fetch.clone().map(|index| {
                    let chunk_key = archive_chunk_key(key, index);
                    async move {
                        match self.get_item(&chunk_key).await? {
                            Some(ArchiveItem::Value(bytes)) => Ok((index, bytes)),
                            _ => Err(MonadChainDataError::Backend(format!(
                                "dynamo archive chunk {index} of {key} is missing"
                            ))),
                        }
                    }
                }))
...
impl crate::external::ExternalBlobReader for DynamoExternalBlobReader {
    fn read_range(
        &self,
        key: &[u8],
        start: usize,
