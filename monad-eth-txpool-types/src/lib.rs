...
mod eth_tx_pool_event_type_tx_serde {
    ...
    pub(super) fn serialize<S>(tx: &MonadTxEnvelope, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
    ...
    pub(super) fn deserialize<'de, D>(deserializer: D) -> Result<MonadTxEnvelope, D::Error>
    where
        D: Deserializer<'de>,
    {
...
impl EthTxPoolIpcTx {
    pub fn new_with_default_priority(tx: MonadTxEnvelope, extra_data: Vec<u8>) -> Self {
        Self {
            tx,
