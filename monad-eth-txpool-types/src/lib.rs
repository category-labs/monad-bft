    ...
    pub fn as_user_string(&self) -> String {
        match self {
            EthTxPoolDropReason::NotWellFormed(err) => match err {
                ...
                StaticValidationError::MaxPriorityFeeTooHigh => "Max priority fee too high",
                StaticValidationError::InitCodeLimitExceeded => "Init code size limit exceeded",
                StaticValidationError::EncodedLengthLimitExceeded => {
                    "Encoded length limit exceeded"
                }
                StaticValidationError::GasLimitTooLow => "Gas limit too low",
                StaticValidationError::GasLimitTooHigh => "Exceeds transaction gas limit",
