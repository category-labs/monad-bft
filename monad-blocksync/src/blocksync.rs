    ...
    pub fn set_secondary_raptorcast_peers(
        ...
        for peer in peers_excl_self {
            let expiry = self
                ...
                .copied()
                .unwrap_or(Round(0));
            let max_expiry = std::cmp::max(expiry, round_span.end);
            self.secondary_raptorcast_peers.insert(peer, max_expiry);
        }
    }
...
mod test {
    ...
    use monad_types::{
        BlockId, Epoch, ExecutionProtocol, Hash, NodeId, Round, RoundSpan, SeqNum,
        GENESIS_BLOCK_ID, GENESIS_SEQ_NUM,
    };
    use monad_validator::{
        epoch_manager::EpochManager,
    ...
    fn make_round_span(start: u64, end: u64) -> RoundSpan {
        RoundSpan {
            start: Round(start),
            end: Round(end),
        }
    ...
    fn pick_peer_rule_1_as_validator() {
        ...
        ctx.set_secondary_raptorcast_peers(vec![sr1, sr2], make_round_span(2, 4), Round(2));
    ...
    fn pick_peer_rule_2_as_fullnode_simple_set() {
        ...
        ctx.set_secondary_raptorcast_peers(vec![sr1, sr2, sr3], make_round_span(2, 4), Round(2));
        let ws = ctx.wrapped_state();
    ...
    fn pick_peer_rule_2_as_fullnode_dynamic_set() {
        ...
        // Round 2
        {
            ws.block_sync.set_secondary_raptorcast_peers(
                vec![p1, p2],
                make_round_span(2, 12),
                Round(2),
            );
