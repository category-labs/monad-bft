...
fn get_max_reserve_balance<CCT, CRT>(chain_config: &CCT, round: Round) -> u128
where
    CCT: ChainConfig<CRT>,
    CRT: ChainRevision,
{
...
fn get_tfm<CCT, CRT>(chain_config: &CCT, round: Round) -> bool
where
    CCT: ChainConfig<CRT>,
    CRT: ChainRevision,
{
    chain_config.get_chain_revision(round).chain_params().tfm
}
    ...
    CRT: ChainRevision,
{
        ...
        SCT: SignatureCollection,
    {
        ...
        let block_index = self.get_block_index(&extending_blocks, &base_seq_num)?;
        let base_max_reserve_balance =
            Balance::from(get_max_reserve_balance(chain_config, block_index.round));
