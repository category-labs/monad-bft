...
async fn bootstrap_peer_entry<ST: CertificateSignatureRecoverable>(
    ...
    let address = match peer.address.parse::<Ipv4Addr>() {
        Ok(address) => address,
        Err(_) => *resolve_domain_v4(&format!("{}:{}", peer.address, peer.auth_port))
            .await
            .ok()
            .flatten()?
            .ip(),
...
async fn resolve_domain_v4(domain: &str) -> Result<Option<SocketAddrV4>, std::io::Error> {
    let dns_response = lookup_host(domain).await?;

    for entry in dns_response {
        if let SocketAddr::V4(addr) = entry {
            return Ok(Some(addr));
        }
    }
