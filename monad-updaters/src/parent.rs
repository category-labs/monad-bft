...
impl ParentExecutorMetrics {
    fn record<T>(&mut self, metric: &'static monad_executor::MetricDef, f: impl FnOnce() -> T) -> T {
...
impl<'a> ParentExecutorMetricsGuard<'a> {
    fn new(metrics: &'a mut ParentExecutorMetrics, guard_metric: &'static monad_executor::MetricDef) -> Self {
        Self {
            metrics,
            guard_metric,
            start: Instant::now(),
