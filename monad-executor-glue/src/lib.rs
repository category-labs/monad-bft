...
#[derive(Deserialize)]
#[serde(bound = "ST: CertificateSignatureRecoverable")]
struct PeerEntrySerdeRepr<ST: CertificateSignatureRecoverable> {
    #[serde(deserialize_with = "deserialize_pubkey::<_, CertificateSignaturePubKey<ST>>")]
    pub pubkey: CertificateSignaturePubKey<ST>,
    #[serde(flatten)]
    pub endpoint: PeerEntryEndpoint,
...
struct PeerEntrySocketAddrEndpoint {
    pub address: SocketAddrV4,
}

#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct PeerEntrySplitEndpoint {
    pub address: Ipv4Addr,
    pub tcp_port: NonZeroU16,
    #[serde(default)]
    pub udp_port: Option<NonZeroU16>,
...
impl<'de, ST: CertificateSignatureRecoverable> Deserialize<'de> for PeerEntry<ST> {
    ...
    where
        D: Deserializer<'de>,
    {
        let repr = PeerEntrySerdeRepr::deserialize(deserializer)?;
        PeerEntry::try_from(repr).map_err(D::Error::custom)
    }
}

impl<ST: CertificateSignatureRecoverable> TryFrom<PeerEntrySerdeRepr<ST>> for PeerEntry<ST> {
    type Error = &'static str;

    fn try_from(repr: PeerEntrySerdeRepr<ST>) -> Result<Self, Self::Error> {
        let tcp_port = repr.endpoint.tcp_port()?;
        let udp_port = repr.endpoint.udp_port()?;

        Ok(PeerEntry {
            pubkey: repr.pubkey,
            address: repr.endpoint.ip(),
            tcp_port,
            udp_port,
...
impl PeerEntryEndpoint {
    ...
    fn udp_port(&self) -> Result<Option<NonZeroU16>, &'static str> {
        match self {
            ...
            Self::SocketAddr(_) => self.tcp_port().map(Some),
        }
    }
}
...
tcp_port = 8003"#,
    ...
    fn peer_entry_rlp_decode_legacy_auth_only_form() {
        ...
        let decoded: PeerEntry<NopSignature> = alloy_rlp::decode_exact(&encoded).unwrap();
        assert_eq!(decoded.address, "127.0.0.3".parse::<Ipv4Addr>().unwrap());
    ...
    fn peer_entry_rlp_decode_legacy_direct_udp_form() {
        ...
        let decoded: PeerEntry<NopSignature> = alloy_rlp::decode_exact(&encoded).unwrap();
        assert_eq!(decoded.address, "127.0.0.4".parse::<Ipv4Addr>().unwrap());
