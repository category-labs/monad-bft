...
use alloy_consensus::{
    ...
    SignableTransaction, TxEip7702, TxLegacy, EMPTY_OMMER_ROOT_HASH,
};
...
fn check_txpool_coherency(
    ...
    let block_txs: Vec<Recovered<MonadTxEnvelope>> = block_under_test
        .validated_txns
        .iter()
    ...
    let txs = proposal
        ...
        .cloned()
        .map(recover_tx)
        .collect::<Vec<Recovered<MonadTxEnvelope>>>();
    let block_policy = TestBlockPolicy::new(GENESIS_SEQ_NUM, 3);
...
fn test_runner(
    ...
    state_backend: NopStateBackend,
    txs: BTreeMap<SeqNum, Vec<Recovered<MonadTxEnvelope>>>,
...
fn make_test_tx(
    ...
    let signature = signer
        ...
        .unwrap();
    let te: MonadTxEnvelope = transaction.into_signed(signature).into();
    recover_tx(te)
}
...
fn insufficient_single_emptying_transaction_inputs() -> (
    BTreeMap<SeqNum, Vec<Recovered<MonadTxEnvelope>>>,
    NopStateBackend,
) {
    let signer = S1;
