...
use crate::{
    ...
    pacing::{BatchLimits, EnqueueError, GlobalPacer, PacingQueue, QueueCost},
};
...
pub(crate) fn spawn_tasks(
    ...
    let mut tx_sockets: Vec<Vec<_>> = (0..worker_count).map(|_| Vec::new()).collect();
    ...
    for (socket_id, socket_addr, ingress_tx) in socket_configs {
        ...
        for worker_sockets in &mut tx_sockets {
            worker_sockets.push((
                socket_id,
    ...
    let global = Arc::new(GlobalPacer::new(mbps_to_bytes_per_second(
        up_bandwidth_mbps,
    )));
    for (worker, (tx_sockets, udp_egress_rx)) in
        tx_sockets.into_iter().zip(udp_egress_rxs).enumerate()
    {
        let global = Arc::clone(&global);
        let metrics = metrics.clone();
        let memory_limit = pacing.max_queued_bytes / worker_count
            + usize::from(worker < pacing.max_queued_bytes % worker_count);
        thread::Builder::new()
...
async fn tx(
    ...
    peer_bandwidth_mbps: u64,
    memory_limit: usize,
    metrics: DataplaneMetrics,
) {
    let tx_sockets: HashMap<UdpSocketId, UdpSocket> = tx_sockets
        .into_iter()
        .map(|(id, socket)| (id, UdpSocket::from_std(socket).unwrap()))
        .collect();
    let mut queue = PacingQueue::with_global_pacer(
        global,
        mbps_to_bytes_per_second(peer_bandwidth_mbps),
        memory_limit,
    );
