...
use super::RecvMsg;
use crate::metrics::{
    RxUdpDataplaneExecutorMetrics, TxUdpDataplaneExecutorMetrics, UdpDataplaneExecutorMetrics,
};
...
pub fn spawn_tasks<MP>(
    ...
    metrics: UdpDataplaneExecutorMetrics<MP>,
) where
    MP: MetricsPolicy,
{
    ...
    let UdpDataplaneExecutorMetrics {
        rx: metrics_rx,
        tx: metrics_tx,
    } = metrics;
...
async fn rx<MP>(
    ...
    udp_ingress_tx: mpsc::Sender<RecvMsg>,
    metrics: RxUdpDataplaneExecutorMetrics<MP>,
) where
    MP: MetricsPolicy,
{
    loop {
...
async fn tx<MP>(
    ...
    up_bandwidth_mbps: u64,
    metrics: TxUdpDataplaneExecutorMetrics<MP>,
) where
    MP: MetricsPolicy,
{
