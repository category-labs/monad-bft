...
use monad_peer_score::dynamic_cap::DynamicCapConfig;
    ...
    S::Identity: Hash + Eq + Clone + Debug + Display,
{
    ...
    fn push_new_identity(
        ...
        let (desired_pool, desired_score) = pool_and_score(self.scorer.score(&id));
        let service_sequence = self.service_sequence;

        match self.pool_mut(desired_pool).push_new(
            id.clone(),
            item,
            desired_score,
            service_sequence,
