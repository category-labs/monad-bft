...
| max_messages_per_identity | 100 | Concurrent messages per identity |
| message_timeout | 100ms | Evict timed-out messages first when full; otherwise evict random |

When the priority reassembly pool enters pressure mode (occupancy >= 80%), per-identity
admission uses a dynamic cap seeded with a bootstrap budget of 10 in-flight messages,
and expands for identities that continuously complete messages.
