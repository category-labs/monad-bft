...
let temp = blake3_keyed_hash(&initiator.chaining_key, &msg.unencrypted_ephemeral);
initiator.chaining_key = blake3_keyed_hash(&temp, &[0x1]);

let temp = blake3_keyed_hash(&initiator.chaining_key, &ecdh(&initiator.ephemeral_private, &responder.static_public));
initiator.chaining_key = blake3_keyed_hash(&temp, &[0x1]);
let key = blake3_keyed_hash(&temp, &[&initiator.chaining_key, &[0x2]].concat());
...
let temp = blake3_keyed_hash(&initiator.chaining_key, &ecdh(&initiator.static_private, &responder.static_public));
initiator.chaining_key = blake3_keyed_hash(&temp, &[0x1]);
let key = blake3_keyed_hash(&temp, &[&initiator.chaining_key, &[0x2]].concat());
...
let temp = blake3_keyed_hash(&responder.chaining_key, &resp.unencrypted_ephemeral);
responder.chaining_key = blake3_keyed_hash(&temp, &[0x1]);

let temp = blake3_keyed_hash(&responder.chaining_key, &ecdh(&responder.ephemeral_private, unencrypted_ephemeral));
responder.chaining_key = blake3_keyed_hash(&temp, &[0x1]);

let temp = blake3_keyed_hash(&responder.chaining_key, &ecdh(&responder.ephemeral_private, &initiator_static));
responder.chaining_key = blake3_keyed_hash(&temp, &[0x1]);

let temp = blake3_keyed_hash(&responder.chaining_key, &preshared_key);
responder.chaining_key = blake3_keyed_hash(&temp, &[0x1]);
let temp2 = blake3_keyed_hash(&temp, &[&responder.chaining_key, &[0x2]].concat());
let key = blake3_keyed_hash(&temp, &[&temp2, &[0x3]].concat());
...
```rust
let temp = blake3_keyed_hash(&chaining_key, &[]);
let temp2 = blake3_keyed_hash(&temp, &[0x1]);
let send_key = blake3_keyed_hash(&temp, &[&temp2, &[0x2]].concat());
let recv_key = blake3_keyed_hash(&temp, &[&temp2, &[0x3]].concat());
```

The initiator and responder derive keys in opposite order, ensuring proper bidirectional communication.

After transport keys have been derived, ephemeral private keys should be zeroized as they are no longer needed.
...
**Note:** AEGIS128L requires a 16-byte nonce. The 8-byte nonce is zero-padded to 16 bytes before use (e.g., nonce value 0x0123456789ABCDEF becomes 0x0123456789ABCDEF0000000000000000).
...
16-47: "RaptorCast Header & Data (atmost 1440 bytes)"
