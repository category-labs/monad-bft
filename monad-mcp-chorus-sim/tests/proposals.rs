...
// proposal index.
type EntriesLog = Arc<Mutex<BTreeMap<(NodeId, Slot), Vec<Entry>>>>;
...
fn build_swarm(
...
) -> (CadenceSwarm<Msg, ChorusDAEvent>, EntriesLog) {
    let entries: EntriesLog = Arc::default();
    ...
    for i in 0..NODES {
        ...
        let observer = move |_now: Timestamp, slot: Slot, data: &SlotFinalization| {
            ...
            let previous = log
                .lock()
                .expect("entries log poisoned")
                .insert((id, slot), slot_entries);
...
fn expected_entry(schedule: &Schedule, slot: Slot, index: usize) -> Entry {
    match schedule.proposers_at(slot).unwrap().proposer(index) {
        Some(proposer) => Entry::Positive(mock_root(&mock_payload(proposer, slot, index))),
        None => Entry::Negative,
    }
}
...
fn expect_entries(
    ...
    let entries = entries.lock().expect("entries log poisoned");
