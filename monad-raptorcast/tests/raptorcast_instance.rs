...
pub fn different_symbol_sizes() {
    ...
    for i in 0..=1 {
        ...
        let validators = EpochValidators {
            validators: BTreeMap::from([(rx_nodeid, Stake(1)), (tx_nodeid, Stake(1))]),
        };
...
> {
    ...
    let pd_builder = NopDiscoveryBuilder::new(known_addresses.clone());
    let mut pd_driver = PeerDiscoveryDriver::new(pd_builder);
    let pd_handle = pd_driver.handle();
    let pd_emit_rx = pd_driver.take_emit_rx();

    let rc = new_defaulted_raptorcast_for_tests::<
        SignatureType,
        MockMessage,
        MockMessage,
        <MockMessage as Message>::Event,
    ...
    // avoid dropping the PD driver for the channels to stay alive
    std::mem::forget(pd_driver);
    rc
}
...
async fn publish_to_full_nodes() {
    ...
    let validator_set = vec![(validator_nodeid, Stake(1))];
