...
enum Commands {
    ...
    Consumer {
        ...
        #[arg(
            long,
            help = "node index (1-based). can be overwritten by NODE_INDEX env variable"
        )]
        index: Option<usize>,
...
fn get_node_index(index_arg: Option<usize>) -> Result<usize> {
    ...
    if node_index == 0 {
        eyre::bail!("node index must be >= 1 (1-based indexing)");
    }
...
fn setup_node(
    ...
    let dataplane = DataplaneBuilder::new(&local_address, UDP_BW).build();
