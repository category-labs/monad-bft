...
/// Raptorcast packet header layout:
...
impl RaptorcastHeader {
    const SIZE: usize = SIGNATURE_SIZE + 2 + 1 + 8 + 8 + MERKLE_HASH_SIZE + 4;
...
pub struct SignedOverData([u8; RaptorcastHeader::SIZE + MERKLE_HASH_SIZE]);

impl SignedOverData {
    fn new(header: &Ref<&[u8], RaptorcastHeader>, merkle_root: &MerkleHash) -> Self {
        let mut data = [0u8; RaptorcastHeader::SIZE + MERKLE_HASH_SIZE];
        data[..RaptorcastHeader::SIZE].copy_from_slice(header.as_bytes());
        data[RaptorcastHeader::SIZE..].copy_from_slice(merkle_root);
        Self(data)
    }
...
const _: () = assert!(
    RaptorcastHeader::SIZE == crate::packet::assembler::HEADER_LEN,
    "RaptorcastHeader size must match HEADER_LEN"
);

/// Raptorcast chunk header layout (follows header and merkle proof):
...
impl RaptorcastChunkHeader {
    const SIZE: usize = MERKLE_HASH_SIZE + 1 + 1 + 2;
}

const _: () = assert!(
    RaptorcastChunkHeader::SIZE == crate::packet::assembler::CHUNK_HEADER_LEN,
...
struct RaptorcastPacket<'a> {
    ...
    chunk_header: Ref<&'a [u8], RaptorcastChunkHeader>,
...
impl<'a> RaptorcastPacket<'a> {
    fn parse(data: &'a [u8]) -> Result<Self, MessageValidationError> {
        ...
        if tree_depth < MIN_MERKLE_TREE_DEPTH || tree_depth > MAX_MERKLE_TREE_DEPTH {
        ...
        if chunk_header_and_payload.len() < RaptorcastChunkHeader::SIZE {
            return Err(MessageValidationError::TooShort);
        }
        let (chunk_header_bytes, payload) =
            chunk_header_and_payload.split_at(RaptorcastChunkHeader::SIZE);
        let chunk_header: Ref<&[u8], RaptorcastChunkHeader> =
        ...
        let payload_offset =
            RaptorcastHeader::SIZE + merkle_proof_len + RaptorcastChunkHeader::SIZE;
    ...
    ST: CertificateSignatureRecoverable,
{
    ...
    if packet.header.version.get() != 0 {
    ...
    let signature = <ST as CertificateSignature>::deserialize(&packet.header.signature)
        .map_err(|_| MessageValidationError::InvalidSignature)?;

    let merkle_root = packet.compute_merkle_root()?;

    let signed_over = SignedOverData::new(&packet.header, &merkle_root);
    ...
    let author = if let Some(&author) = signature_cache.get(&signed_over) {
        ...
        signature_cache.put(signed_over, author);
        author
    };
...
mod tests {
    ...
    #[test]
    #[ignore] // ignoring because this test is very slow
