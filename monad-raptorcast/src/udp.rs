...
mod tests {
    ...
    use super::{
        ChunkSignatureVerifier, GroupId, MessageIdentifier, MessageValidationError, UdpState,
    };
    use crate::{
        ...
        for (_to, mut aggregate_message) in messages {
            while !aggregate_message.is_empty() {
                ...
                assert_eq!(
                    ...
                    MessageIdentifier::AppMessageHash(HexBytes(
                        app_message_hash.0[..20].try_into().unwrap()
                    )),
                );
