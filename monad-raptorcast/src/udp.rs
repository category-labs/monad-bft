...
pub fn round_robin_order(
    ...
    while let Some((sock, range)) = outbound_gso_idx.pop_front() {
        ...
        let end = range.start + segment_size;
...
// such a message.
pub fn build_messages_with_length<ST, F>(
    key: &ST::KeyPairType,
    segment_size: u16,
    app_message: Bytes,
    app_message_len: u32,
...
where
    ...
    F: FnOnce(&Bytes, usize, u16, VecDeque<(SocketAddr, Range<usize>)>) -> Vec<(SocketAddr, Bytes)>,
{
...
mod tests {
    ...
    proptest! {
        #[test]
        fn test_round_robin_message_sizes(message_size in 1..=(DEFAULT_SEGMENT_SIZE as usize * 100)) {
            const SEED: u64 = 42;

            let (key, validators, known_addresses) = validator_set();
            let epoch_validators = validators.view_without(vec![&NodeId::new(key.pubkey())]);

            let app_message: Bytes = vec![1_u8; message_size].into();
            let app_message_len: u32 = app_message.len().try_into().expect("message too big");
            ...
            let mut rng2 = StdRng::seed_from_u64(SEED);
            let messages_legacy = build_messages_with_length(
                &key,
                DEFAULT_SEGMENT_SIZE,
                app_message,
                app_message_len,
            ...
            let total_len_rr: usize = messages_round_robin.iter().map(|(_, msg)| msg.len()).sum();
            let total_len_legacy: usize = messages_legacy.iter().map(|(_, msg)| msg.len()).sum();
            prop_assert_eq!(total_len_rr, total_len_legacy);

            for (_, msg) in &messages_round_robin {
                prop_assert!(msg.len() <= DEFAULT_SEGMENT_SIZE as usize);
            }
        }
    }
}
