...
pub enum MessageValidationError {
    UnknownVersion(u16),
    TooShort, // app message or chunk shorter than expected
    TooLong,  // app message or chunk longer than expected
    InvalidSignature,
    InvalidTreeDepth,
    InvalidMerkleProof,
    InvalidChunkId,
...
mod tests_regular {
    ...
    use crate::{
        ...
        util::{
            BroadcastMode, BuildTarget, FullNodeGroupMap, HexBytes, PrimaryBroadcastGroup,
    ...
    fn test_roundtrip() {
        ...
        let group = PrimaryBroadcastGroup::of_epoch(EPOCH, &self_id, &self_id, &group_map).unwrap();
    ...
    fn test_broadcast_bit() {
        ...
        let group_map = make_group_map(&validators);
        let primary_group =
            PrimaryBroadcastGroup::of_epoch(EPOCH, &self_id, &self_id, &group_map).unwrap();
        ...
        let round = Round(1);
        let mut full_node_group_map = FullNodeGroupMap::default();
        full_node_group_map
            .try_insert(SecondaryGroupAssignment::new(
                self_id,
                RoundSpan::new(round, Round(round.0 + 1000)).unwrap(),
                full_nodes,
            ))
            .unwrap();
        let secondary_group =
            SecondaryBroadcastGroup::of_round(round, &self_id, &self_id, &full_node_group_map)
                .unwrap();
...
mod tests_deterministic {
    ...
    fn build_packets(
        ...
        MessageBuilder::<SignatureType>::new(key)
            ...
            .build_vec(app_message, &BuildTarget::Raptorcast(group))
    ...
    fn test_deterministic_end_to_end_decode_and_rebroadcast() {
        ...
        let group_map = make_group_map(&validators);
        let group =
            PrimaryBroadcastGroup::of_epoch(EPOCH, &sender_id, &sender_id, &group_map).unwrap();
    ...
    fn test_deterministic_reproducibility() {
        ...
        let proposer_group =
            PrimaryBroadcastGroup::of_epoch(EPOCH, &proposer_id, &proposer_id, &group_map).unwrap();
        ...
        // validator set).
        let validator_group =
            PrimaryBroadcastGroup::of_epoch(EPOCH, &validator_id, &validator_id, &group_map).unwrap();
