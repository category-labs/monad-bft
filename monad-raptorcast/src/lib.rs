    ...
    fn tcp_build_and_send(
        ...
        match self.known_addresses.get(to) {
            ...
            Some(address) => {
                ...
                if let Err(err) = self.compression_algo.compress(&app_message, &mut compressed_app_message) {
                ...
                let mut signed_message = BytesMut::zeroed(SIGNATURE_SIZE + compressed_app_message.len());
...
>(
    ...
    if let Ok(inbound) = InboundRouterMessage::<M, CertificateSignaturePubKey<ST>>::deserialize(bytes) {
        Ok(inbound)
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            ...
            let deserialized_app_messages = decoded_app_messages.into_iter().filter_map(
                |(from, decoded)| match handle_message::<ST, M, CA>(&decoded, &this.compression_algo) {
        ...
        while let Poll::Ready((from_addr, message)) = pin!(this.dataplane.tcp_read()).poll_unpin(cx)
        {
            ...
            let deserialized_message = match handle_message::<ST, M, CA>(&app_message_bytes, &this.compression_algo) {
                Ok(message) => message,
                Err(err) => {
