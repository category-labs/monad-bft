...
impl<'a> RaptorcastPacketV1<'a> {
    ...
    pub fn validate_tree_depth(&self) -> Result<(), InvalidChunk> {
        ...
        let depth = self.header.tree_depth()?;

        let Some(min_depth) = deterministic::calc_tree_depth(
            encoding_scheme,
            app_message_len,
            MIN_VALIDATOR_SET_SIZE,
        ) else {
        ...
        let Some(max_depth) = deterministic::calc_tree_depth(
            encoding_scheme,
            app_message_len,
            ...
            InvalidChunk::InvalidMerkleTreeDepth
        );
    ...
    // caller must validate tree_depth is valid before calling this
    fn segment_len(&self) -> usize {
        RAPTORCAST_HEADER_V1_SIZE
            + self.merkle_proof.len() * MERKLE_HASH_SIZE
            + self.chunk_header_and_payload.len()
    }
    ...
    fn validate_chunk_meta(
        ...
        let tree_depth = self.header.tree_depth()?;
        let layout = deterministic::PacketLayout::new(segment_len, tree_depth);
        ...
        let mut chunk_id_cap =
            layout.num_symbols(app_message_len, deterministic::DEFAULT_REDUNDANCY);
        let broadcast_mode = self.header.broadcast_mode()?;
        let encoding_scheme = self.header.encoding_scheme()?;
        if matches!(encoding_scheme, EncodingScheme::Deterministic25(_)) {
            // TODO: use more accurate estimation for number of rounding chunks
            chunk_id_cap = chunk_id_cap.saturating_add(MAX_VALIDATOR_SET_SIZE);
        }
        let chunk_id = self.chunk_header.chunk_id.get() as usize;
