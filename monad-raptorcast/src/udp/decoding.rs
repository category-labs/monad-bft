...
use crate::util::{compute_hash, AppMessageHash, HexBytes, NodeIdHash, Validator};
...
type ValidatorSet<PT> = BTreeMap<NodeId<PT>, Validator>;
    ...
    PT: PubKey,
{
    fn new(config: DecoderCacheConfig) -> Self {
        let stake_based_quota = SlotByStake::default();
        let fixed_quota = FixedSlot::default();
...
struct SoftQuotaCache<PT: PubKey> {
    ...
    quota_policy: Box<dyn QuotaPolicy<PT>>,
}
...
impl<PT: PubKey> SoftQuotaCache<PT> {
    ...
    pub fn insert(
        ...
        // keys are considered expired. so the cache is validly full
        // and we need to make space for the new entry. this may
        // happen when there are many authors. now we randomly evict a
        // cache entry to compensate for the new entry.
...
impl<PT: PubKey> CacheStore<PT> {
    fn new(max_size: usize) -> Self {
        assert!(max_size > 1, "max_size must be greater than 1");

        Self {
            max_size,
            store: IndexMap::with_capacity(max_size),
        }
    }
...
impl ChronologicalIndex {
    ...
    fn filter_by_unix_ts_threshold(&mut self, threshold: UnixTimestamp) -> Vec<CacheKey> {
        ...
        while let Some((unix_ts, key)) = self.time_index.first() {
    ...
    fn filter_by_epoch_threshold(&mut self, threshold: Epoch) -> Vec<CacheKey> {
        ...
        let mut filtered = vec![];
        while let Some((epoch, key)) = self.epoch_index.first() {
            if *epoch > threshold {
                break;
            }
            filtered.push(key.clone());
...
impl Default for SlotByStake {
    fn default() -> Self {
        Self {
            min_slots: 10,
            non_validator_slots: 5,
        }
    }
}
...
impl<PT: PubKey> QuotaPolicy<PT> for SlotByStake {
    fn calc_quota(
        ...
        let Some(validator) = validator_set.get(&message.author) else {
            return self.non_validator_slots.min(cache_size);
        };

        // quota = proportional to stake
        let total_stake: u64 = validator_set.values().map(|v| v.stake.0).sum();
        let stake_fraction = (validator.stake.0 as f64) / (total_stake as f64);
...
mod test {
    ...
    fn add_validators(set: &mut ValidatorSet<PT>, ids: &[u64]) {
        let stake = Stake(100);
        for id in ids {
    ...
    fn make_cache(max_authors: usize, max_messages_per_author: usize) -> DecoderCache<PT> {
    ...
    fn make_symbols(app_message: &Bytes, author: NodeId<PT>) -> Vec<ValidatedMessage<PT>> {
    ...
    fn try_decode_all<'a>(
        ...
        validator_set: Option<&ValidatorSet<PT>>,
