...
mod test {
    ...
    fn test_time_based_eviction() {
        ...
        assert_eq!(cache.pending_len(MessageTier::P2P), 9);
    }
    ...
    fn test_stake_based_quota_allocation() {
        ...
        // Author 0 has 90% of the stake, so should get 90% of the cache slots.
        // Author 1 has 10% of the stake, so should get 10% of the cache slots.
        add_validators(&mut validator_set, &[0], 90);
        add_validators(&mut validator_set, &[1], 10);
        ...
        all_symbols_part_1.shuffle(&mut rand::thread_rng());
        ...
        let res = try_decode_all(&mut cache, &context, all_symbols_part_1.iter())
            .expect("Decoding should succeed");
        assert_eq!(res.len(), 10); // cache size is capped to 10, so
                                   // only up to 10 messages can be decoded

        let author_msg_count = res.into_iter().counts_by(|sym| sym.0);
        assert_eq!(author_msg_count[&node_id(0)], 9);
        assert_eq!(author_msg_count[&node_id(1)], 1);
    }
    fn try_decode_all<'a>(
