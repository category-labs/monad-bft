    ...
    AP: AuthenticationProtocol,
{
    ...
    pub async fn recv(&mut self) -> Result<AuthRecvMsg<AP::PublicKey>, AP::Error> {
        match (&mut self.authenticated, &mut self.non_authenticated) {
            (Some(authenticated), Some(non_authenticated)) => {
                tokio::select! {
            ...
            (Some(authenticated), None) => {
                let result = authenticated.recv().await;
                if let Ok(ref msg) = result {
            ...
            (None, Some(non_authenticated)) => {
                let msg = non_authenticated.recv().await;
            ...
            (None, None) => panic!("DualSocketHandle requires at least one UDP socket"),
        }
    }
