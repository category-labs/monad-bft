    ...
    PD: PeerDiscoveryAlgo<SignatureType = ST>,
{
    ...
    pub fn write_to_peer(
        ...
        {
            if let Some(session_addr) = authenticated
                .auth_protocol
                .get_socket_by_public_key(&public_key)
            {
                ...
                authenticated.write(session_addr, payload);
                return;
            }
            ...
            match authenticated.try_send_via_wireauth(&public_key, wireauth_addr, payload) {
                ...
                Err(returned_payload) => {
                    self.metrics
                        ...
                        .add(returned_payload.len() as u64);
...
mod tests {
    ...
    fn test_dual_tcp_sigauth_only() {
        ...
        local.block_on(&rt, async {
            ...
            name_records.insert(sender.node_id, sender.create_name_record(false));
            name_records.insert(receiver.node_id, receiver.create_name_record(false));
    ...
    fn test_dual_tcp_wireauth_only() {
        ...
        local.block_on(&rt, async {
            ...
            let mut name_records = HashMap::new();
            name_records.insert(sender.node_id, sender.create_name_record(true));
            name_records.insert(receiver.node_id, receiver.create_name_record(true));
    ...
    fn test_dual_tcp_wireauth_bidirectional() {
        ...
        local.block_on(&rt, async {
            let node_a = NodeInfo::new(1);
            let node_b = NodeInfo::new(2);

            let mut name_records = HashMap::new();
            name_records.insert(node_a.node_id, node_a.create_name_record(true));
            name_records.insert(node_b.node_id, node_b.create_name_record(true));
            ...
            let pd_a = create_pd(name_records.clone());
            let pd_b = create_pd(name_records.clone());

            let (mut socket_a, _control_a) = create_dual_tcp_socket(&node_a, pd_a);
            let (mut socket_b, _control_b) = create_dual_tcp_socket(&node_b, pd_b);

            let node_a_id = node_a.node_id;
            let node_b_id = node_b.node_id;

            let (tx_a_recv, mut rx_a_recv) = tokio::sync::mpsc::channel::<Bytes>(10);
            let (tx_b_recv, mut rx_b_recv) = tokio::sync::mpsc::channel::<Bytes>(10);
            let (tx_b_ready, mut rx_b_ready) = tokio::sync::mpsc::channel::<()>(1);
            let (tx_stop, mut rx_stop) = tokio::sync::broadcast::channel::<()>(2);
            let mut rx_stop_b = tx_stop.subscribe();
            ...
            tokio::task::spawn_local(async move {
                ...
                loop {
                    tokio::select! {
                        _ = rx_stop.recv() => break,
                        result = socket_a.recv() => {
                            match result {
                                Ok(msg) => {
                                    let _ = tx_a_recv.send(msg.payload).await;
                                }
                                Err(e) => {
                                    tracing::warn!(?e, "node_a recv error");
                                }
                            }
                        }
                    }
            ...
            tokio::task::spawn_local(async move {
                loop {
                    tokio::select! {
                        _ = rx_stop_b.recv() => break,
                        result = socket_b.recv() => {
                            match result {
                                Ok(msg) => {
                                    let payload_str = String::from_utf8_lossy(&msg.payload);
                                    if payload_str == "init_from_a" {
                                        let _ = tx_b_ready.send(()).await;
                                    }
                                    let _ = tx_b_recv.send(msg.payload).await;

                                    socket_b.write_to_peer(&node_a_id, Bytes::from("reply_from_b"), None);
                                }
                                Err(e) => {
                                    tracing::warn!(?e, "node_b recv error");
                                }
                            }
                        }
                    }
            ...
            let timeout = tokio::time::timeout(Duration::from_secs(10), async {
                rx_b_ready.recv().await.expect("b should receive init");

                let b_msg = rx_b_recv.recv().await.expect("b should have received");
                assert_eq!(
                    String::from_utf8_lossy(&b_msg),
                    "init_from_a",
                    "b should receive init message from a"
                );

                let a_msg = rx_a_recv.recv().await.expect("a should receive reply");
                assert_eq!(
                    String::from_utf8_lossy(&a_msg),
                    "reply_from_b",
                    "a should receive reply from b over the session b received"
                );
            })
            .await;
