...
pub struct UdpMessage {
    ...
    pub payload: Bytes,
    pub stride: usize,
    // 0 - highest priority, 255 - lowest priority, None - unspecified
...
where
    ...
    R: rand::Rng,
{
    use generator::{Replicated, Partitioned};
    ...
    let generator: Box<dyn ChunkGenerator<_>> = match build_target {
        ...
        BuildTarget::Broadcast(ref nodes_view) => {
            Box::new(Replicated::from_broadcast(nodes_view.iter().copied().collect()))
        }
    ...
    let packets = assemble::<ST, _, _>(
        ...
        &*generator,
        peer_lookup,
        assembler::BatchingStrategy::GsoConcat,
    );
...
mod tests {
    ...
    fn test_compatibility() {
        ...
        for num_pkts in [0, 1, 100, 2000] {
            let mut trail_lens =
                (2..(DEFAULT_SEGMENT_LEN - 1)).choose_multiple(rng, 10);
            ...
            for trail_len in trail_lens {
                for redundancy in [1.0, 1.3, 3.0, 7.0] {
                    let mut app_msg = BytesMut::zeroed(DEFAULT_SEGMENT_LEN * num_pkts + trail_len);
    ...
    fn assert_compatible_eq(expected: &Packets, actual: &Packets) {
        ...
        if option_env!("SCRUTIZE_PACKET") != Some("1") {
