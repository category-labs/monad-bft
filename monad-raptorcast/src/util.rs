        ...
        let rebroadcast_group = match msg_group_id {
            ...
            GroupId::Secondary(msg_round) => {
                ...
                if msg_round >= group.round_span.start && msg_round < group.round_span.end {
                    ...
                    // FIXME: this is the backward compatible branch, which can
                    // be hit if full node is upgraded before the validator.
                    // It'll be removed after the upgrade
                    group_queue.first_key_value()?.1
                }
            }
        };
