...
use super::{
    serializer::ChunkSerializer, BuildError, ChunkGenerator, Collector, PeerAddrLookup, Result,
    UdpMessage,
};
use crate::{
    message::MAX_MESSAGE_SIZE,
    packet::serializer::OperationMode,
...
pub(crate) fn assemble<ST, CG, CS, PL>(
    key: &ST::KeyPairType,
...
where
    ...
    CS: ChunkSerializer<CertificateSignaturePubKey<ST>>,
    PL: PeerAddrLookup<CertificateSignaturePubKey<ST>>,
{
    ...
    match serializer.operation_mode() {
        // serialize all chunks together
        OperationMode::Full => {
            // step 6. write header, chunk_header and signature for
            // each merkle batch.
            for merkle_batch in merkle_batches(&mut chunks, layout) {
                merkle_batch.write_header::<ST>(layout, key, &header_buf)?;
            }

            // step 7. assemble udp messages
            serializer.serialize_into(collector, chunks, layout);
        }

        // serialize each merkle batch as soon as they finish signing
        // to reduce latency.
        OperationMode::Stream => {
            for mut batch in owned_merkle_batches(chunks, layout) {
                ...
                // step 7. assemble udp messages
                serializer.serialize_into(collector, batch, layout);
            }
        }
    }
...
pub(crate) struct Chunk<PT: PubKey> {
    // these fields are made public for chunk_serializer to consume an
    // owned Chunk easily without extra hassles.
    pub(super) chunk_id: usize,
    pub(super) recipient: Recipient<PT>,
...
impl<PT: PubKey> Recipient<PT> {
    ...
    #[cfg(test)]
    pub(super) fn set_addr(&self, addr: SocketAddr) {
...
impl<'a, PT: PubKey> MerkleBatch<'a, PT> {
    ...
    #[cfg(test)]
    pub(super) fn write_chunk_headers(&mut self, layout: PacketLayout) -> Result<()> {
        for (leaf_index, chunk) in self.chunks.iter_mut().enumerate() {
            let leaf_index = u8::try_from(leaf_index).map_err(|_| BuildError::MerkleTreeTooDeep)?;
            chunk.write_chunk_header(layout, leaf_index)?;
        }
