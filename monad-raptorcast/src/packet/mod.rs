    ...
    F: FnMut(monad_dataplane::UnicastMsg),
{
    ...
    // warn on drop if overdue
    deadline: Instant,
    // used solely for printing metrics
    total_bytes: usize,
}
    ...
    F: FnMut(monad_dataplane::UnicastMsg),
{
    fn drop(&mut self) {
        ...
        if now > self.deadline {
            let elapsed = now - self.deadline + Self::TIMEOUT;
            tracing::warn!(
                "UdpMessageBatcher dropped after deadline exceeded, total bytes flushed: {}, took {:?}",
                self.total_bytes,
                elapsed
            );
        }
    ...
    F: FnMut(monad_dataplane::UnicastMsg),
{
    fn push(&mut self, item: UdpMessage) {
        ...
        self.total_bytes += item.payload.len();
