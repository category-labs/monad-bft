...
impl<PT: PubKey> ChunkGenerator<PT> for StakeBasedWithRC<PT> {
    ...
    fn generate_chunks(
        ...
        let mut ic = vec![0usize; num_validators]; // ic := floor(o)
        let mut rc = vec![false; num_validators]; // rc := o - floor(o)

        let mut total_num_chunks = 0;
        for (i, o) in obligations.enumerate() {
            ic[i] = o as usize;
            rc[i] = o.fract() > 0.0;
            total_num_chunks += ic[i] + dbg!(rc[i] as usize);
        }

        let mut buffer = BytesMut::zeroed(dbg!(total_num_chunks) * layout.segment_len());
...
mod tests {
    ...
    use super::StakeBasedWithRC;
    use crate::{
        packet::{ChunkGenerator as _, PacketLayout},
        util::Redundancy,
    };
    ...
    fn test_stake_with_rc() {
        ...
        for scale in [
            ...
            let total_chunks = 22; // after rounding chunks

            let generator = StakeBasedWithRC::from_validator_set(validator_set).with_rc_priority(1);

            let chunks = generator
                .generate_chunks(message_len, redundancy, DEFAULT_LAYOUT)
                .expect("should generate chunks");
            assert_eq!(chunks.len(), total_chunks);

            let rounding_chunks = chunks.iter().filter(|c| c.priority == Some(1)).count();
            assert_eq!(rounding_chunks, 3);

            let unique_recipients: HashSet<_> =
                chunks.iter().map(|c| *c.recipient.node_id()).collect();
            assert_eq!(unique_recipients.len(), 4);

            let mut counts: Vec<usize> = unique_recipients
                .into_iter()
                .map(|nid| {
                    chunks
                        .iter()
                        .filter(|c| *c.recipient.node_id() == nid)
                        .count()
                })
                .collect();
            counts.sort_unstable();

            assert_eq!(counts, vec![2, 5, 7, 8])
        }
    }
}
