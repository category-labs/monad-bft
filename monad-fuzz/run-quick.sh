...
fuzz() (
    ...
    TIMEOUT_QUICK="${TIMEOUT_QUICK:-5m}"
    ...
    if [[ -z "$(find "$corpus_dir" -maxdepth 1)" ]]; then
    ...
    # allow running fuzzer without system tuning that requires root
    ...
    timeout --preserve-status --kill-after=5s --signal=INT "$TIMEOUT_QUICK" \
            cargo afl fuzz "${afl_args[@]}" 2>&1 | sed "s/^/[$target]/" || true
)
