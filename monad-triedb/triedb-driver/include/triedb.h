...
typedef struct validator_set_result
{
    int result;
    struct validator_set validator_set;
} validator_set_result;

#pragma pack(pop)

void free_valset(validator_set);

validator_set_result read_valset(triedb *, size_t block_num, uint64_t requested_epoch);
