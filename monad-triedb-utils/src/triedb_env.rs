    ...
    async fn get_call_frames(&self, block_key: BlockKey) -> Result<Vec<Vec<u8>>, String> {
        ...
        match request_receiver.await {
            Ok(Some(rlp_call_frames)) => {
                ...
                let mut grouped_frames: BTreeMap<u64, BTreeMap<u8, Vec<u8>>> = BTreeMap::new();
                for TraverseEntry { key, value } in rlp_call_frames {
                    ...
                    // first 8 bytes is txn_index
                    let txn_index = u64::from_be_bytes(key[0..8].try_into().unwrap_or_default());
                    // 9th byte is chunk_index (u8)
                    let chunk_index = key[8];
                    grouped_frames
                        .entry(txn_index)
                        .or_default()
