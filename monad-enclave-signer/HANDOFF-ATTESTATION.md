...
workstream of the enclave-signer PoC. The rest of the PoC is built and validated;
...
into a **enclave signer** running inside an **AMD SEV-SNP confidential VM** on the
...
Read `monad-enclave-signer/README.md` first — architecture, crates, provisioning
model, build/run. This doc assumes it.

## What's already done (don't redo)

- `monad-enclave-signer-proto` — wire protocol, pooled client, vsock+unix transport.
- `monad-secp` / `monad-bls` — `Local | Remote` enums (feature `enclave-signer`);
  every caller unchanged; tests pass.
- `monad-signer` (enclave service), `monad-keyloader` (provisioning), `monad-signer-bench`.
- `monad-node` wired with `--enclave-signer-vsock-*` flags.
...
### 2. Implement the report fetch — `monad-enclave-signer/src/attest.rs`
Replace the stub `attestation_report(nonce, pubkeys)`:
- Add the `sev` crate (Linux-only target dep) to `monad-enclave-signer/Cargo.toml`.
...
### 3. Verify on the keyloader — `monad-enclave-signer/src/bin/monad-keyloader.rs`
The client method `EnclaveSigner::attest(&nonce)` already exists. Add:
...
*response payload* (not the request) and update `EnclaveSigner::attest` to parse it.

## Build / test / run
```bash
# Linux binaries via Docker (no host toolchain):
./monad-enclave-signer/scripts/build-linux.sh        # -> target-linux/release/

# tests (off-SNP):
cargo test -p monad-enclave-signer-proto
cargo test -p monad-enclave-signer
