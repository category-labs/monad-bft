...
pub fn eth_call(
    ...
    for (address, state_override_object) in state_override_set {
        ...
        if state_override_object.balance.is_some() {
        ...
        if state_override_object.nonce.is_some() {
            cxx_state_override_set.as_mut().set_override_nonce(
                &cxx_address,
                &state_override_object.nonce.unwrap().as_limbs()[0],
            );
        }

        if state_override_object.code.is_some() {
        ...
        if state_override_object.state.is_some() {
            let override_state = state_override_object.state.as_ref().unwrap();
        ...
        } else if state_override_object.state_diff.is_some() {
            let override_state_diff = state_override_object.state_diff.as_ref().unwrap();
