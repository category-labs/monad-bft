...
pub(crate) struct SealingVotePool<V: IsVote> {
    ...
    /// stays a read
    sealed: OnceCell<StrongQc<V, V::Scope>>,
}
...
impl<V: IsVote> SealingVotePool<V> {
    ...
    fn add_vote(&mut self, node_id: NodeId, msg: VoteMsg<V, V::Scope>) {
    ...
    fn try_form_strong_qc(&self, validator_data: &ValidatorData) -> Option<&StrongQc<V, V::Scope>> {
