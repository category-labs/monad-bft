...
impl FastPath {
    ...
    pub(crate) fn handle_da_event(&mut self, event: ChorusDAEvent) -> Option<FastBlock> {
        ...
        avail.ingest(event);
        if let Some(cert) = avail.equiv_cert() {
            ...
            match evidence {
                ProposalEvidence::FallbackSignedEntry(entry) => {
                    if let Some(header) = entry.header() {
                        ...
                        if self.availability[j].header_for(&header.root).is_none() {
                            // header never seen. report this to
                            // DA. q: this may not be necessary.
                        ...
                        let avail = &mut self.availability[j];
                        avail.record_header(header.clone());
                        if let Some(equiv_cert) = avail.equiv_cert() {
    ...
    pub(crate) fn on_commit_vote_deadline(&mut self) -> CommitVoteDeadlineOutcome {
        ...
        // a fresh positive fallback entry obliges us to re-disseminate
