...
struct Args {
    ...
    #[arg(
        long,
        help = "Optional direct UDP authenticated port. Requires --authenticated-udp-port"
    )]
    direct_udp_auth_port: Option<u16>,
...
fn main() {
    ...
    let name_record = match (args.authenticated_udp_port, args.direct_udp_auth_port) {
        (Some(authenticated_udp_port), Some(direct_udp_auth_port)) => {
        ...
        (None, Some(_)) => {
            panic!("--direct-udp-auth-port requires --authenticated-udp-port")
        }
        (None, None) => {
    ...
    if let Some(direct_udp_auth_port) = args.direct_udp_auth_port {
        println!("direct_udp_auth_port = {}", direct_udp_auth_port);
    }
    println!(
