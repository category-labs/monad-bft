...
use crate::{
    ...
    PeerDiscoveryEvent, PeerLookup, PeerLookupRequest, PeerLookupResponse, PeerTable, Ping, Pong,
};
...
impl<ST: CertificateSignatureRecoverable> PeerDiscoveryAlgoBuilder for NopDiscoveryBuilder<ST> {
    ...
    fn build(
        ...
        let peer_table: HashMap<_, _> = self
            .known_addresses
            .into_iter()
        ...
        let state = NopDiscovery {
            peer_table: RwLock::new(PeerTable::from(peer_table)),
    ...
    ST: CertificateSignatureRecoverable,
{
    ...
    fn update_peers(&mut self, peers: Vec<PeerEntry<ST>>) -> Vec<PeerDiscoveryCommand<ST>> {
        ...
        let mut peer_table = self.peer_table.read().unwrap().name_records();
        for peer in peers {
        ...
        *self.peer_table.write().unwrap() = PeerTable::from(peer_table);
