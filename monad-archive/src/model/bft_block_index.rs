...
const BFT_INDEX_MARKERS_PREFIX: &str = "bft_index_markers/";
const BFT_BLOCK_NUM_PREFIX: &str = "bft_block_num/";
const BFT_BLOCKS_PREFIX: &str = "bft_blocks/";
...
fn bft_index_marker_key_from_seq_num(seq_num: &SeqNum) -> String {
    format!("{}{}", BFT_INDEX_MARKERS_PREFIX, seq_num.0)
}
...
fn bft_block_key_from_id(id: &BlockId) -> String {
    format!("{}{}", BFT_BLOCKS_PREFIX, hex::encode(id.0))
}
...
impl BftBlockIndex {
    ...
    pub async fn index_bft_headers(
        ...
        let handles = known_committable_heads.iter().map(|marker| {
            ...
            tokio::spawn(async move {
                this.index_sub_chain(marker, other_sub_chain_tips, max_num_per_batch)
                    .await
            })
        });
    ...
    async fn ensure_markers(&self, concurrency: usize) -> Result<HashSet<IndexedRangeMarker>> {
        ...
        let candidates = self
            ...
            .scan_prefix_with_max_keys(BFT_BLOCKS_PREFIX, concurrency * 2)
            .await?
            .into_iter()
    ...
    async fn index_sub_chain(
        ...
        while !other_sub_chain_tips.contains(&marker.end_id) {
            ...
            for _ in 0..max_num_per_batch {
                ...
                self.put_index(&current_header).await?;
    ...
    pub async fn get_index(&self, num: u64) -> Result<BlockId> {
        let key = bft_block_num_key_from_seq_num(&SeqNum(num));
        let value = self
            .kv
            .get(&key)
            .await?
            .ok_or_eyre("Block number not found")?;
        let id = BlockId(Hash::decode(&mut &value[..]).unwrap());
        Ok(id)
    }
    ...
    async fn write_marker(&self, range: &IndexedRangeMarker) -> Result<()> {
        ...
        let key = bft_index_marker_key_from_seq_num(&SeqNum(range.start_num));
        self.kv
            .put(key, bytes)
            .await
    ...
    pub async fn fetch_header_by_id(&self, hash: &BlockId) -> Result<BftBlockHeader> {
        let key = bft_block_key_from_id(hash);
    ...
    async fn put_index(&self, header: &BftBlockHeader) -> Result<()> {
        put_index(&self.kv, header).await
    }
}
...
pub async fn put_index(kv: &KVStoreErased, header: &BftBlockHeader) -> Result<()> {
    let encoded_hash = hex::encode(header.get_id().0).into_bytes();
    let key = bft_block_num_key_from_seq_num(&header.seq_num);
    kv.put(key, encoded_hash).await
}
