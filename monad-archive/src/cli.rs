...
impl ArchiveArgs {
    pub async fn build_block_data_archive(&self, metrics: &Metrics) -> Result<BlockDataArchive> {
        Ok(BlockDataArchive::new(self.build_store(metrics).await?))
    }

    /// Open the raw block KV store for this archive (no [`BlockDataArchive`]
    /// wrapper).
    pub async fn build_store(&self, metrics: &Metrics) -> Result<KVStoreErased> {
