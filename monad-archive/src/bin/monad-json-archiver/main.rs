...
fn block_to_json(
    block: &AlloyBlock<alloy_consensus::TxEnvelope, Header>,
...
async fn process_block(
    ...
    let json_doc = block_to_json(&ethereum_block, &receipts, &traces);
...
mod tests {
    ...
    fn create_test_block(
        ...
        AlloyBlock<alloy_consensus::TxEnvelope, Header>,
        BlockReceipts,
        BlockTraces,
    ) {
        ...
        // Convert to ethereum block (strip sender)
        let ethereum_block = AlloyBlock {
            header: block.header,
            body: alloy_consensus::BlockBody {
                transactions: block
        ...
        (ethereum_block, receipts, traces)
    }
