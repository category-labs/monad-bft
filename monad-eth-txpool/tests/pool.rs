            ...
            TxPoolTestEvent::CreateProposal {
                ...
                let proposal = pool
                    .create_proposal(
                        ...
                        maybe_preloaded_addresses.as_ref(),
                        &eth_block_policy,
                        &state_backend,
                        &MockChainConfig::DEFAULT,
                    )
    ...
    run_simple(
        ...
        [
            ...
            TxPoolTestEvent::CreateProposal {
                ...
                maybe_preloaded_addresses: None,
                expected_txs: vec![],
...
#[test]
fn test_create_proposal_with_preloaded_addresses() {
    let tx1 = make_legacy_tx(S1, BASE_FEE, GAS_LIMIT, 0, 10);
    let tx2 = make_legacy_tx(S2, BASE_FEE, GAS_LIMIT, 0, 10);
    let tx3 = make_legacy_tx(S3, BASE_FEE, GAS_LIMIT, 0, 10);
    let tx4 = make_legacy_tx(S4, BASE_FEE, GAS_LIMIT, 0, 10);

    run_simple(
        &[S1, S2, S3, S4],
        [
            TxPoolTestEvent::InsertTxs {
                txs: vec![(&tx1, true), (&tx2, true), (&tx3, true), (&tx4, true)],
                expected_pool_size_change: 4,
            },
            TxPoolTestEvent::CreateProposal {
                base_fee: BASE_FEE_PER_GAS,
                tx_limit: 128,
                gas_limit: 10 * GAS_LIMIT,
                byte_limit: PROPOSAL_SIZE_LIMIT,
                maybe_preloaded_addresses: Some(HashSet::from([
                    secret_to_eth_address(S1),
                    secret_to_eth_address(S2),
                ])),
                expected_sender_gas: None,
