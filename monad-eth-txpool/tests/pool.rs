    ...
    let state_backend = {
        let nonces = if let Some(nonces) = nonces_override {
            ...
            events
                ...
                .map(|address| (address, AccountState::max()))
                .collect()
        };
...
fn run_simple<const N: usize>(account: &[B256], events: [TxPoolTestEvent<'_>; N]) {
    run_custom(
        ...
        Some(BTreeMap::from_iter(
            account
                .iter()
                .map(|a| (secret_to_eth_address(*a), AccountState::max())),
        )),
        events,
    );
}
        ...
        [
            ...
            TxPoolTestEvent::Block(Arc::new({
                ...
                move |pool| {
                    ...
                    txs.retain(|tx| {
                        !pending.contains(tx.tx_hash()) && !tracked.contains(tx.tx_hash())
                    });
...
fn test_eip7702_valid_authorization_changes_nonce() {
    ...
    run_custom(
        make_test_block_policy,
        Some(BTreeMap::from_iter([
            (secret_to_eth_address(S1), AccountState::max()),
            (secret_to_eth_address(S2), AccountState::max()),
        ])),
        [
            TxPoolTestEvent::InsertTxs {
                txs: vec![(&tx1, true), (&tx2, true)],
...
fn test_eip7702_authorization_nonce_lower() {
    ...
    run_custom(
        ...
        Some(BTreeMap::from_iter([
            (secret_to_eth_address(S1), AccountState::max()),
            (secret_to_eth_address(S2), AccountState::max_with_nonce(1)),
        ])),
        [
            TxPoolTestEvent::InsertTxs {
                txs: vec![(&tx1, true), (&tx2, true)],
