...
impl TrackedTxList {
    pub fn try_new(
        ...
        for tx in &mut txs {
            let Some(tx) = limits.prepare_add_tx_to_new_address(tx) else {
                continue;
            };

            let Some(tx) = this.try_insert_tx(event_tracker, tx, last_commit_base_fee) else {
                continue;
            };
        ...
        for tx in txs {
            let Some(tx) = limits.prepare_add_tx_to_existing(tx) else {
                continue;
            };
