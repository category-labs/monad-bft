...
use monad_eth_types::{ExtractEthAddress, MonadTxEnvelope};
...
pub struct EthTxPoolForwardingManager<S> {
    ingress_batch: Option<Vec<(S, MonadTxEnvelope)>>,
...
impl<S> EthTxPoolForwardingManagerProjected<'_, S> {
    pub fn add_ingress_txs(&mut self, txs: Vec<(S, MonadTxEnvelope)>) {
        let Self {
            ingress_batch,
            ingress_waker,
            ..
    ...
    pub fn add_egress_txs<'a>(&mut self, txs: impl Iterator<Item = &'a MonadTxEnvelope>) {
        let Self {
            egress,
            egress_waker,
            ..
...
mod test {
    ...
    fn generate_tx(nonce: u64) -> MonadTxEnvelope {
        make_legacy_tx(S1, BASE_FEE_PER_GAS, 100_000, nonce, 0)
    }

    fn generate_ingress_tx(nonce: u64) -> ((), MonadTxEnvelope) {
        ((), generate_tx(nonce))
    }
