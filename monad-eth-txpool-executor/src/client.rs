    ...
    CRT: ChainRevision,
{
    ...
    fn enqueue_forwarded(&mut self, forwarded: Vec<ForwardedTxs<SCT>>) {
        ...
        for ForwardedTxs { sender, txs } in forwarded {
            ...
            for (index, tx) in txs.into_iter().enumerate() {
                match self.forwarded_ingress.push(sender, tx) {
                    Ok(()) => {
    ...
    fn poll_forwarded_send(&mut self, cx: &mut std::task::Context<'_>) {
        ...
        self.pending_forwarded_send = Some(PendingForwardedSend::new(self.forwarded_tx.clone()));
    ...
    CRT: ChainRevision,
{
    ...
    fn metrics(&self) -> ExecutorMetricsChain<'_> {
        ExecutorMetricsChain::from(&self.metrics).push(self.forwarded_ingress.metrics())
    }
}
