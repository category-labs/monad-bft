...
impl<'a> Rate<'a> {
    ...
    fn rate(&mut self, elapsed: f64) -> f64 {
        let curr = self.val();
...
pub struct MetricsReporter {
    ...
    // Gauges — f64 to preserve sub-1/sec rates (e.g. sparse errors)
    committed_tps: Gauge<f64>,
    send_errors_ps: Gauge<f64>,
    send_errors_nonce_too_low_ps: Gauge<f64>,
    send_errors_overloaded_ps: Gauge<f64>,
    send_errors_replacement_ps: Gauge<f64>,
    send_errors_other_ps: Gauge<f64>,
    sent_tps: Gauge<f64>,
    rpc_calls_ps: Gauge<f64>,
    rpc_calls_error_ps: Gauge<f64>,
    contracts_deployed_ps: Gauge<f64>,
...
impl MetricsReporter {
    pub fn new(
        ...
        let reporter = Self {
            ...
            committed_tps: meter.f64_gauge("committed_tps").build(),
            send_errors_ps: meter.f64_gauge("send_errors_ps").build(),
            send_errors_nonce_too_low_ps: meter.f64_gauge("send_errors_nonce_too_low_ps").build(),
            send_errors_overloaded_ps: meter.f64_gauge("send_errors_overloaded_ps").build(),
            send_errors_replacement_ps: meter.f64_gauge("send_errors_replacement_ps").build(),
            send_errors_other_ps: meter.f64_gauge("send_errors_other_ps").build(),
            sent_tps: meter.f64_gauge("sent_tps").build(),
            rpc_calls_ps: meter.f64_gauge("rpc_calls_ps").build(),
            rpc_calls_error_ps: meter.f64_gauge("rpc_calls_error_ps").build(),
            contracts_deployed_ps: meter.f64_gauge("contracts_deployed_ps").build(),
    ...
    pub fn emit_zeros(&self) {
        ...
        self.sent_tps.record(0.0, label);
        self.send_errors_ps.record(0.0, label);
        self.send_errors_nonce_too_low_ps.record(0.0, label);
        self.send_errors_overloaded_ps.record(0.0, label);
        self.send_errors_replacement_ps.record(0.0, label);
        self.send_errors_other_ps.record(0.0, label);
        self.rpc_calls_ps.record(0.0, label);
        self.rpc_calls_error_ps.record(0.0, label);
        self.contracts_deployed_ps.record(0.0, label);
    ...
    fn report_metrics(&self, elapsed: f64, rates: &mut Rates) {
        ...
        self.committed_tps
            .record(rates.committed_txs.rate(elapsed), label);
        self.send_errors_ps
            .record(rates.send_errors.rate(elapsed), label);
        self.send_errors_nonce_too_low_ps
            .record(rates.send_errors_nonce_too_low.rate(elapsed), label);
        self.send_errors_overloaded_ps
            .record(rates.send_errors_overloaded.rate(elapsed), label);
        self.send_errors_replacement_ps
            .record(rates.send_errors_replacement.rate(elapsed), label);
        self.send_errors_other_ps
            .record(rates.send_errors_other.rate(elapsed), label);
        self.sent_tps.record(rates.txs_sent.rate(elapsed), label);
        self.rpc_calls_ps
            .record(rates.rpc_calls.rate(elapsed), label);
        self.rpc_calls_error_ps
            .record(rates.rpc_calls_error.rate(elapsed), label);
        self.contracts_deployed_ps
            .record(rates.contracts_deployed.rate(elapsed), label);
