        ...
        for _ in 0..num_connections {
            ...
            tasks.push(tokio::spawn(async move {
                ...
                loop {
                    ...
                    // Helper function implemented as a plain async fn inside the block to avoid lifetime issues
                    async fn ws_call(
                        ...
                        loop {
                            let msg = read.next().await.ok_or_else(|| "ws closed".to_string())
                    ...
                    let (ws_chain, http_chain) = tokio::join!(
                        ws_call(&mut write, &mut read, id, "eth_chainId", serde_json::json!([])),
                        async {
                            http_client
                    ...
                    match (ws_chain, http_chain) {
                        (Ok(ws_val), Ok(http_u64)) => {
                            let ws_u64: U64 = serde_json::from_value(ws_val).expect("invalid ws chainId");
                    ...
                    let (ws_bn, http_bn) = tokio::join!(
                        ws_call(&mut write, &mut read, id, "eth_blockNumber", serde_json::json!([])),
                        async {
                            http_client
                    ...
                    let block_number: U64 = match (ws_bn, http_bn) {
                        (Ok(ws_val), Ok(http_u64)) => {
                            let _ws_u64: U64 = serde_json::from_value(ws_val).expect("invalid ws blockNumber");
                    ...
                    match (ws_block, http_block) {
                        (Ok(ws_val), Ok(http_blk)) => {
                            let mut ws_blk: alloy_rpc_types_eth::Block = serde_json::from_value(ws_val).expect("invalid ws block");
                    ...
                    let (ws_bal, http_bal) = tokio::join!(
                        ws_call(&mut write, &mut read, id, "eth_getBalance", serde_json::json!([random_addr, block_number])),
                        async {
                            http_client
                    ...
                    match (ws_bal, http_bal) {
                        (Ok(ws_val), Ok(http_u256)) => {
                            let ws_u256: U256 = serde_json::from_value(ws_val).expect("invalid ws balance");
                    ...
                    let (ws_nonce, http_nonce) = tokio::join!(
                        ws_call(&mut write, &mut read, id, "eth_getTransactionCount", serde_json::json!([random_addr, block_number])),
                        async {
                            http_client
                    ...
                    match (ws_nonce, http_nonce) {
                        (Ok(ws_val), Ok(http_u256)) => {
                            let ws_u256: U256 = serde_json::from_value(ws_val).expect("invalid ws nonce");
                    ...
                    let (ws_gas, http_gas) = tokio::join!(
                        ws_call(&mut write, &mut read, id, "eth_estimateGas", serde_json::json!([estimate_req, block_number])),
                        async {
                            http_client
                    ...
                    match (ws_gas, http_gas) {
                        (Ok(ws_val), Ok(http_u256)) => {
                            let ws_u256: U256 = serde_json::from_value(ws_val).expect("invalid ws gas");
...
impl RpcWsCompare {
    ...
    pub async fn run(&self) {
        ...
        let (ws_stream, _) = connect_async(&self.ws_url.to_string()).await.expect("Failed to connect");
        let (mut write, mut read) = ws_stream.split();
        write.send(Message::Text("{ \"id\": 1, \"jsonrpc\": \"2.0\", \"method\": \"eth_subscribe\", \"params\": [\"newHeads\"] }".into())).await.expect("failed to send message");
        Self::wait_for_subscription_id(&mut read, 1).await.expect("failed to get newHeads subscription ID");
        ...
        loop {
            ...
            let rpc_header = block_receiver.recv().await.take().expect("rpc block not found").header;

            if ws_header.number != rpc_header.number {
                panic!("block number mismatch websocket {} rpc {}", ws_header.number, rpc_header.number);
            }
            debug!(
    ...
    async fn wait_for_subscription_id(read: &mut SplitStream<tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>>, json_rpc_id: u32) -> Option<String> {
