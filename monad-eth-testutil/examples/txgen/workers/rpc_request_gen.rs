...
async fn compare_results<F1, F2, R, E>(ws_future: F1, rpc_future: F2) -> Result<R, ()>
where
    ...
    F2: Future<Output = Result<R, RpcError<E>>>,
{
    ...
    match (ws_result, rpc_result) {
        (Ok(ws_val), Ok(rpc_val)) => {
            let ws_val: R = serde_json::from_value(ws_val).expect("invalid ws gas");
            assert_eq!(ws_val, rpc_val);
            Ok(ws_val)
        }
        _ => {
            warn!("parity failed; continuing");
            Err(())
        }
    }
}
