...
use crate::{
    ...
    prelude::{rpc_request_gen::{RpcWalletSpam, RpcWsCompare}, *},
    shared::{
...
fn run_traffic_gen(
    ...
    if traffic_gen.spam_rpc {
        let spammer = RpcWalletSpam::new(read_client.clone(), config.ws_url().expect("WS URL is not valid"));
        tasks.push(critical_task("Spammer", tokio::spawn(async move {
            spammer.run().await
        })).boxed());
    }

    if traffic_gen.compare_rpc_ws {
        let compare_rpc_ws = RpcWsCompare::new(read_client.clone(), config.ws_url().expect("WS URL is not valid"));
        tasks.push(critical_task("Compare RPC WS", tokio::spawn(async move {
            compare_rpc_ws.run().await
        })).boxed());
    }

    Ok(tasks.into_iter().chain([
