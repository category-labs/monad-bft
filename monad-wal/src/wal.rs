    ...
    M: Serializable + Deserializable,
{
    ...
    #[cfg(feature = "nightly")]
    pub fn push_vectored(&mut self, message: &M) -> Result<(), WALError<M>> {
        let msg_buf = message.serialize();
        let len_buf = msg_buf.len().to_be_bytes().to_vec();
        let bufs = &mut [IoSlice::new(&len_buf), IoSlice::new(&msg_buf)];
        self.file_handle.write_all_vectored(bufs)?;
        self.file_handle.flush()?;
        Ok(())
    }
