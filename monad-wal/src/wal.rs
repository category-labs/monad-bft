...
pub(crate) const DEFAULT_CHUNKS: usize = 2;
    ...
    M: Serializable<Bytes> + Debug,
{
    ...
    pub fn build(self) -> Result<WALogger<M>, WALError> {
        ...
        if self.chunk_size <= EVENT_HEADER_LEN as u64 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "wal chunk_size must be larger than the event header",
            )
            .into());
        }

        let mut discovered = discover_chunks(&self.file_path)?;
        while discovered.len() > self.chunks {
            let stale_chunk = discovered.remove(0);
            fs::remove_file(stale_chunk.path)?;
        }
            ...
            Some(chunk) => {
                if chunk.used_len > self.chunk_size {
                    return Err(io::Error::new(
                        ...
                        "existing wal chunk exceeds configured chunk_size",
                    )
                    .into());
                }
                reopen_chunk(chunk)?
    ...
    M: Serializable<Bytes> + Debug,
{
    ...
    fn rotate(&mut self) -> Result<(), WALError> {
        ...
        let next_current = if self.rotated.len() + 1 < self.chunks {
...
fn reopen_chunk(chunk: DiscoveredChunk) -> Result<ActiveChunkState, WALError> {
