...
impl Bencher {
    ...
    #[cfg(feature = "nightly")]
    fn append_vectored(&mut self) {
        self.logger.push_vectored(&self.data).unwrap()
    }
...
fn bench_block_vec(c: &mut Criterion) {
    ...
    c.bench_function("block_vectorized", |b| b.iter(|| bencher.append_vectored()));
}
...
#[cfg(feature = "nightly")]
fn bench_vote_vec(c: &mut Criterion) {
    let mut bencher = Bencher::new(VOTE_SIZE);

    c.bench_function("vote_vectorized", |b| {
        b.iter(|| {
            for _ in 0..N_VALIDATORS {
                bencher.append_vectored()
            }
        })
    });
}
