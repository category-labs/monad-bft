// TestEvent
// impl ser/de for test event
// generate a vector of events, log and replay
// maybe create a vector state which wraps the wal
...
mod test {
    ...
    // impl executor::state for VecState
    impl State for VecState {
        ...
        fn init(config: Self::Config) -> (Self, Vec<monad_executor::Command<Self>>) {
            // open the wal file
            // optimization: copy the wal
            let (wal, events): (WALogger<_>, Vec<TestEvent>) =
                WALogger::new(config.wal_path).unwrap(); // should truncate the file of incomplete message
        ...
        fn update(&mut self, event: Self::Event) -> Vec<monad_executor::Command<Self>> {
